# session 7e93206e-e981-4b7f-985d-dada4c3e18b1

- cwd: /Users/housibo/Documents/知乎黑客松
- origin: subagent depth:1
- parent: session-55bc2520-62f6-43cd-8c4a-f97717376fc6
- createdAt: 2026-09-11T13:54:04.827Z
- agentPreset: standard

- subagent: 核实 llm_wiki 机制原文 (deepseek-flash)


## 👤 USER

你是事实核查员。目标：核实一个开源项目 `nashsu/llm_wiki` 的具体机制宣称，只依据一手来源（GitHub README、raw 文件、Karpathy 的原始 gist/推文、项目文档），不接受二手转述或搜索摘要。

背景：有人读了中文媒体（AI 内参 260911 期）对 nashsu/llm_wiki 的介绍，需要判断其中哪些说法在项目原文里站得住。该项目被描述为：把 Andrej Karpathy（安德烈·卡帕西，OpenAI 创始成员、前特斯拉 AI 总监）公开的一段 prompt 模式（gist）做成桌面 app。

请逐条核实下面 9 条宣称，并给出结论：
1. llm_wiki 是桌面 app（技术栈是不是 Tauri/Electron？），其设计来自 Karpathy 公开的 prompt 或 gist；给出 Karpathy 原文链接与原话。
2. 主张「知识编译一次、持续维护」（compiled once and kept current），明确对比 RAG 的「每次从零检索再回答」（retrieve-and-answer from scratch）。
3. 权属三分：原始材料不可变、由人策划，模型只读；整理出的 wiki 页面由模型完全拥有，人只读；规则与意图层由人机共同演化。
4. 判断内容相关性的权重设置：把「共享同一份来源」（shared source / 同源）的权重设得比「显式互相链接」（explicit links）更高。
5. 做社区发现（community detection）：用了什么算法（Louvain / Leiden / 其他）、有没有「凝聚度」（cohesion / modularity）指标，是已实现功能还是 roadmap。
6. 图谱洞察功能：提示「意外连接」（unexpected connections）与「知识缺口」（knowledge gaps），例如 degree ≤ 1 的孤立页面。
7. 知识库会反过来向用户提问（系统生成问题/提示）。
8. 人的可用动作被收窄为三个：建页、深挖、跳过（create page / dig deeper / skip）。
9. 有人类在环的异步评审（async review）与深度研究（deep research）功能。

方法要求：
- 先 web_search 定位仓库，再 web_fetch 抓取 https://github.com/nashsu/llm_wiki 与 README 原始文件（例如 https://raw.githubusercontent.com/nashsu/llm_wiki/main/README.md，若分支不是 main 就试 master 或从仓库页找默认分支）；必要时抓 docs/ 目录下的设计文档。
- 每条宣称给出：结论（原文支持／原文未提及／原文相反／尚未核实）＋英文原句引用（逐字，含所在文件与标题）＋URL。
- 若 README 是中文，照样逐字引用并注明语言。若某项功能只在 roadmap / TODO / issue 里出现，必须标明「规划中，未实现」。
- 顺带记录：仓库 star 数、license、主要语言、最近提交时间（如页面可见）。

输出格式（markdown，简洁，不要复述任务）：
### 仓库与 Karpathy 源头
（事实 + 链接）
### 九条宣称核查表
| # | 宣称 | 结论 | 原文引用（英文/中文逐字） | 来源 URL |
### 与转述不一致或夸大的地方
（逐条列出，没有就写「未发现」）
### 未能核实的项与原因

只报告你实际抓到的内容；抓不到就写「抓取失败＋原因」，绝不臆测。
...
{
  "schema_version": 1,
  "generated_at": "2026-09-11T13:54:05.242828+00:00",
...
I'll start by locating the repository and fetching primary sources.

- 🔧 web_search: {"queries": ["nashsu llm_wiki github", "llm_wiki Karpathy gist wiki prompt desktop app"]}
- 🔧 web_fetch: {"url": "https://github.com/nashsu/llm_wiki"}

## 🤖 ASSISTANT

GitHub main domain is blocked by DNS here. Let me try raw file hosts and direct fetch.

- 🔧 web_fetch: {"url": "https://raw.githubusercontent.com/nashsu/llm_wiki/main/README.md"}
- 🔧 bash: {"command": "curl -sSL --max-time 30 -o /tmp/llmwiki_readme.md -w \"http=%{http_code} size=%{size_download} url=%{url_effective}\\n\" https://raw.githubusercont …

## 🤖 ASSISTANT

curl works (web_fetch's DNS resolver is sandboxed). Fetching primary sources directly.

- 🔧 bash: {"command": "mkdir -p /tmp/llmwiki && cd /tmp/llmwiki && cp /tmp/llmwiki_readme.md README.md && wc -l README.md && echo \"--- repo meta ---\" && curl -sSL --max …
- 🔧 read: {"file_path": "/tmp/llmwiki/README.md"}

## 🤖 ASSISTANT

README captured. Now fetching Karpathy's gist, repo metadata, and cloning for source-level verification.

- 🔧 bash: {"command": "cd /tmp/llmwiki && echo \"=== GIST RAW ===\" && curl -sSL --max-time 30 -o karpathy_llm_wiki.md -w \"http=%{http_code} size=%{size_download}\\n\" h …
- 🔧 bash: {"command": "cd /tmp/llmwiki && timeout 300 git clone --depth 1 https://github.com/nashsu/llm_wiki.git repo 2>&1 | tail -5; echo \"exit=$?\"; ls repo 2>/dev/nul …
- 🔧 read: {"file_path": "/tmp/llmwiki/karpathy_llm_wiki.md"}
- 🔧 bash: {"command": "cd /tmp/llmwiki && git clone --depth 1 https://github.com/nashsu/llm_wiki.git repo 2>&1 | tail -5; echo \"---\"; ls repo | head -40; echo \"--- las …

## 🤖 ASSISTANT

Repo cloned (HEAD `e808211`, v0.6.11). Now verifying source-level details and metadata.

- 🔧 bash: {"command": "cd /tmp/llmwiki && echo \"=== diff raw README vs clone ===\" && diff <(cat README.md) repo/README.md && echo \"IDENTICAL\"; echo; echo \"=== plans/ …
- 🔧 bash: {"command": "cd /tmp/llmwiki && echo \"=== github page stars/license ===\"; curl -sSL --max-time 40 -H \"User-Agent: Mozilla/5.0\" https://github.com/nashsu/llm …
- 🔧 bash: {"command": "cd /tmp/llmwiki && echo \"=== LICENSE / lang / commit from page ===\"; grep -oE '\"licenseInfo\":\\{\"[^}]*\\}' repo_page.html | head -2; grep -oE  …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== signal weights ===\"; grep -rn \"SOURCE_OVERLAP\\|sourceOverlap\\|DIRECT_LINK\\|directLink\\|ADAMIC\\|adamicAdar …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== LOUVAIN / cohesion / modularity ===\"; grep -rni \"louvain\\|modularity\\|cohesion\" src src-tauri --include=*.t …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== review action types ===\"; grep -rni \"create_page\\|createPage\\|'skip'\\|\\\"skip\\\"\\|deepResearch\\|deep_re …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== ingest.ts 2085-2135 ===\"; sed -n '2085,2135p' src/lib/ingest.ts; echo; echo \"=== ingest.ts 2320-2365 ===\"; se …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== graph-insights degree <=1 ===\"; grep -n \"degree\" src/lib/graph-insights.ts | head -20; echo; echo \"=== graph …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== immutable / co-evolve / ownership in templates ===\"; grep -rni \"immutable\\|co-evolve\\|coevolve\\|never modif …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== question generation ===\"; grep -rni \"ask the user\\|questions to investigate\\|open questions\\|suggest.*quest …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== project.rs scaffolding 20-140 ===\"; sed -n '20,140p' src-tauri/src/commands/project.rs", "description": "Read p …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== README_CN key phrases ===\"; grep -n \"编译\\|RAG\\|权属\\|不可变\\|共同演化\\|共同进化\\|深挖\\|跳过\\|建页\\|提问\\|凝聚\\|社区发现\\|意外连接\ …
- 🔧 bash: {"command": "cd /tmp/llmwiki && echo \"=== gist page meta ===\"; curl -sSL --max-time 40 -H \"User-Agent: Mozilla/5.0\" https://gist.github.com/karpathy/442a6bf …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== README_CN 65-90 ===\"; sed -n '65,90p' README_CN.md; echo; echo \"=== README_CN 138-200 (graph/community/insight …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== 'question' in src (non-test) ===\"; grep -rni \"question\" src --include=*.ts --include=*.tsx | grep -vi \"test\ …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== lint.ts prompt 260-320 ===\"; sed -n '255,320p' src/lib/lint.ts", "description": "Read lint prompt"}
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== tauri conf ===\"; ls src-tauri/ | head; grep -m6 -E '\"(productName|version|identifier)\"' src-tauri/tauri.conf. …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== chat-panel 1350-1400 ===\"; sed -n '1350,1400p' src/components/chat/chat-panel.tsx; echo; echo \"=== interaction …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== AskUserQuestion tool def ===\"; sed -n '3150,3185p' src-tauri/src/agent/runtime.rs; echo; echo \"=== review sugg …
- 🔧 bash: {"command": "cd /tmp/llmwiki && echo \"=== releases atom ===\"; curl -sSL --max-time 40 https://github.com/nashsu/llm_wiki/releases.atom -o rel.atom -w \"http=% …
- 🔧 bash: {"command": "cd /tmp/llmwiki && echo \"=== modularity explicit ===\"; grep -rni \"modularity\" repo/src repo/src-tauri 2>/dev/null | head -5; echo \"(none above …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== schema/purpose editability ===\"; grep -rni \"schema.md\\|purpose.md\" src/components --include=*.tsx | head -15 …
- 🔧 bash: {"command": "cd /tmp/llmwiki && echo \"=== gh available? ===\"; command -v gh && gh auth status 2>&1 | head -3; echo; echo \"=== gist dates ===\"; python3 - <<' …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== byte share by extension (my own count, src only) ===\"; for e in ts tsx rs css md json; do n=$(find src src-taur …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== wiki-editor save ===\"; grep -n \"writeFile\\|savePage\\|onSave\" src/components/editor/wiki-editor.tsx | head - …
- 🔧 bash: {"command": "cd /tmp/llmwiki && python3 - <<'EOF'\nimport re,html\ns=open('gist.html',encoding='utf-8',errors='ignore').read()\ni=s.find('relative-time')\nprint …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== WikiEditor usage/onSave wiring ===\"; grep -rn \"WikiEditor\" src/components --include=*.tsx | head -6; echo \"- …
- 🔧 bash: {"command": "cd /tmp/llmwiki && python3 - <<'EOF'\nimport re,html\ns=open('gist.html',encoding='utf-8',errors='ignore').read()\nfor m in re.finditer(r'datetime= …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== preview-panel handleSave ===\"; grep -n \"handleSave\" -A12 src/components/layout/preview-panel.tsx | head -30;  …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== deep-research / review files ===\"; ls -la src/lib/deep-research.ts src/lib/review*.ts src/stores/review-store.t …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== wiki.write_page path validation ===\"; sed -n '352,400p' src-tauri/src/agent/runtime.rs | head -30; echo; echo \ …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== fs writeFile validation ===\"; grep -rn \"writeFile\\|createMissingWikiPage\" src/commands/fs.ts | head -12; ech …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && echo \"=== preview-panel 110-135 ===\"; sed -n '110,135p' src/components/layout/preview-panel.tsx; echo; echo \"=== about-s …
- 🔧 bash: {"command": "cd /tmp/llmwiki/repo && grep -n \"electron\" package-lock.json | head -5; echo \"(above: only transitive mentions, if any)\"", "description": "Chec …

## 🤖 ASSISTANT

All claims verified against primary sources. Writing the report.

- 🔧 write: {"file_path": "/Users/housibo/Documents/知乎黑客松/llm_wiki_事实核查.md", "content": "# nashsu/llm_wiki 九条宣称事实核查\n\n核查日期：2026-09-11。方法：`web_fetch` 对 github.com / raw.git …
- 🔧 send_message: {"agent_id": "session-55bc2520-62f6-43cd-8c4a-f97717376fc6", "message": "事实核查完成：nashsu/llm_wiki 九条宣称（仅一手来源）。完整报告已写入 `llm_wiki_事实核查.md`（工作区根目录）。方法说明：web_fetch 对  …
- 🔧 present: {"files": [{"path": "llm_wiki_事实核查.md", "description": "nashsu/llm_wiki 九条宣称的一手来源事实核查报告（含逐字引文与 URL）"}]}
