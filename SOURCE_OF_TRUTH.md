...
| 要找什么 | 去哪里 | 当前状态／备注 |
|---|---|---|
| **概念地图 v2（2026-09-13 全量替换旧 194 概念池）** ★ | `knowledge/概念地图-260913/`（topics 1156 · dependencies 930 · relations 4370 · clusters 23 · manifest · schema）＋ `knowledge/概念wiki-260913/`（1156 页概念页 ＋ `[[双链]]` ＋ index/log/README） | **当前概念唯一真源**。三源＝用户 Notion 概念库 509（含逐页正文）＋ 飞书 Context Engineering 28 篇 ＋ Harness Engineering 30 篇；源快照冻结在 `evidence/概念源-260913/`（含 `SHA256SUMS.txt`），Notion 全程只读。地图照 <https://github.com/withmarbleapp/os-taxonomy>，链接层照 <https://github.com/nashsu/llm_wiki>。管线 `scripts/cm-*.mjs`（七步，两次 LLM 都带缓存可断点重跑）；体检 `node scripts/cm-validate.mjs`。**旧 194 概念池退役**，只在 `build-shell.mjs --legacy` 里留回退分支。已知缺口（170 个孤立点、17 个无入链页、169 条已策展关系只解析进 16 条、Notion 非 AI 概念是否裁掉待裁决）见下一行 |
| **概念地图 v2 完整说明**（三源、管线、数字、验收、没做的） | `docs/概念地图v2-三源替换-20260913.md` | 09-13 建；查「概念从哪来」「为什么只有 930 条边」「Notion 里那些非 AI 概念为什么在」先读它 |
