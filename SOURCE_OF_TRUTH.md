...
| **公网 Demo（赛制必交件）** ✅ | **<https://hou-152.github.io/>**（裸域名，自动跳 135）→ <https://hou-152.github.io/zhisuoqi-135/>（发布仓库 <https://github.com/hou-152/zhisuoqi-135>）。**离线单文件**：<https://github.com/hou-152/zhisuoqi-135/raw/main/index.html>（双击就开，不需要联网） | **09-12 08:3x 上线**。单文件、零依赖、零服务端：194 概念 / 353 依赖 / 10 条策展线 / 边预判 / 15 个 skill 全部烘死。构建器 `scripts/build-public.mjs`（源 `prototype/知所栖-壳.html`）→ `deploy/zhisuoqi-135/index.html`；验收 `scripts/check-public.mjs`（**用假域名 + `--host-resolver-rules` 才能测出公网分支**，本地 127.0.0.1 会走服务端那条路）。对话三条路：本地服务端 → 访客自带 DeepSeek key（只进他自己 localStorage）→ 录制回放（**明说不是本次回答**）。发布仓库**只放构建产物**，不含 `.private/`、不含源码仓库 |
| **公网 Demo 的备用通道（Codex 发）** | `deploy/zhisuoqi/dist/135.html` | 09-12 放好，但 **`zhisuoqi.sibohou792.chatgpt.site/135.html` 现在还是 404** —— 我推不上去（远端要 Codex 环境的凭证），Codex 必须 commit + push 才会存在。⚠️ 09-12 我在消息里把它说成了可用地址，导致所有者打开 404，已更正 |
| **根站（防裸域名 404）** | <https://hou-152.github.io/>（仓库 <https://github.com/hou-152/hou-152.github.io>；源 `deploy/root-site/`） | 09-12 建。GitHub Pages 规则：没有 `<用户名>.github.io` 这个仓库时，裸域名根路径返回 404。这个仓库只做一件事——立刻跳到 `/zhisuoqi-135/` |
...
| **桌面版（Electron）：能写真文件的那一版** | `docs/桌面版-为什么要有它-20260912.md`；代码 `app/`（`main.js` · `package.json` · `README.md`）；构建 `scripts/build-app.mjs`；验收 `scripts/test-app.mjs` | **09-12 10:xx 建**，起因是所有者问「为什么不变成 app」——此前我把「必交件是链接」当成了「所以不做 app」，是偷懒（他最早的原话就是「先做 APP，不行再做网站」）。服务本体抽成 `scripts/serve-lib.mjs`，**命令行版与桌面版 import 同一份**。桌面版比网页版多的只有能写真文件：`~/Documents/知所栖-135/学习记录.json`（所有者 05:09「保留它本地嘛」的落点）+ `产物/<课题>.md`（**dbs-learning-beta 的 SKILL.md 写死的那条路径第一次被兑现**）。打包 macOS arm64：306MB 解包 / 122MB zip，**未签名未公证**。打包版 `/api/health` 回 `llm:false`——**不内置 key**，构建时自检扫 `.private/` 类文件，命中即失败 |
| **本地服务（唯一一份）** | `scripts/serve-lib.mjs` | 09-12 从 `serve-135.mjs` 抽出来。`serve-135.mjs` 现在是命令行壳（端口 5180），`app/main.js` 是 Electron 壳（随机端口 + 窗口）。接口：`/api/health` `/api/skills` `/api/search` `/api/llm` `/api/save` `/api/data`。**修掉一个真缺陷**：`/api/save` 原来把路径里的 `..` **过滤掉**而不是拒绝，越界尝试会被静默改写成目录内的一次成功写入；现在直接拒绝 + `join` 后再校验 |
| **外壳：Linear 式三栏（导航 | 列表 | 详情）** | `docs/外壳-Linear三栏-20260912.md`；源 `scripts/shell.template.html` | **09-12 09:4x 重做**，起因是所有者贴出真 Linear 截图说「没有 linear 的味道」。原版是「全屏画布 + 滑出抽屉」——**Linear 的骨架是三栏，中间那一栏（可扫列表）根本没有**，而那一栏才是它的形状；「味道」是结构问题不是配色问题。现在：导航 224（5 项各带计数 + 分组行）/ 列表 302（一行一个东西，行高 30）/ 主区画布 + 工具行 / 详情 412 **停靠让位不盖图**。配色收到 `#08090a` `#1b1c1e` `#5e6ad2`，去光晕去渐变。顺带修：`.dot` 是 inline 导致底部色点不显示；`resize()` 没减掉 412 导致详情打开时右两列被切；左栏 e2/e5 都写成 tags.length 导致两个数字同为 10 |
| **分类层：不问「这讲的是什么」，问「这个概念要你怎么处理它」** | `docs/分类层-要你怎么处理它-20260912.md`；产物 `evidence/concept-classes-20260912.json`（生成器 `scripts/classify-concepts.mjs`） | **09-12 09:xx 建**，起因是所有者「概念分类一般吧，我觉得」。原 10 条主题线是**主题聚类**——任何模型都能吐，对学习没用。改成按**验收方式**四分：**能算的 69**（复述机制）/ **能判的 57**（写出自己的判据与代价）/ **能用的 27**（给一个真用过的例子）/ **只能认的 41**（**不设验收**——世界观没有机制可复述，硬考就是假学习）。**这同时修掉了倒逼层的真 bug**：此前 194 条用同一个验收标准，把世界观也拉来考。默认横轴换成这四列，主题/来源两档保留。公网无模型时 `能判的`/`能用的` **直接按没过记**（机械检查看不见「你的判据」和「你的例子」） |
