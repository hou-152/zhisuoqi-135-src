...
| **代码仓库** | 本项目已 `git init`（09-12） | **不写死提交数**（此前写「4 个」→ 实测 8 → 再实测 17，连续过期两次；要数字自己跑 `git rev-list --count HEAD`）；`.private/` `.local/` `.mimosa/` `.DS_Store` `deploy/zhisuoqi/` `.trash/` `.playwright-cli/` 均已 gitignore；**未推 GitHub** |
