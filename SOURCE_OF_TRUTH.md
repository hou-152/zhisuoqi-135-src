...
| **概念地图 v2（2026-09-13 全量替换旧 194 概念池；同日过完全量审核）** ★ | `knowledge/概念地图-260913/`（topics **936** · dependencies **531** · relations 3333 · clusters 21 · manifest · schema）＋ `knowledge/概念wiki-260913/`（936 页概念页 ＋ `[[双链]]` ＋ index/log/README） | **当前概念唯一真源**。三源＝用户 Notion 概念库 509（含逐页正文）＋ 飞书 Context Engineering 28 篇 ＋ Harness Engineering 30 篇；源快照冻结在 `evidence/概念源-260913/`（含 `SHA256SUMS.txt`），Notion 全程只读。地图照 <https://github.com/withmarbleapp/os-taxonomy>，链接层照 <https://github.com/nashsu/llm_wiki>。管线 `scripts/cm-*.mjs`（七步，两次 LLM 都带缓存可断点重跑）；体检 `node scripts/cm-validate.mjs`。**旧 194 概念池退役**，只在 `build-shell.mjs --legacy` 里留回退分支。**已按「只要 AI 的」过滤**：1156 → 856（删 300 条非 AI，逐条带理由留 `evidence/cm-260913/07-ai-filter.json`，没物理删除）。**09-13 又并入第四个源「AI 内参 260912」**：10 篇概念辞典 88 条 → 8 条撞上已有、80 条新增 → **856 → 936**；壳里内参那一栏的 88 张概念卡现在 **88/88 全部能点回地图**（`evidence/cm-260913/08-neican-merge.json`）。**每条依赖都带审核结论**（yes / weak 已降 soft / reversed-fixed；判否的边留 `rejected` 可翻案）。最终抽检：定义有原文支持 88%、**无编造**、领域 100%、依赖边成立 87.5%。已知缺口（264 个孤立点、14 个无入链页）见下两行 |
