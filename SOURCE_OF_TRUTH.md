...
| **壳：知识体系 + 内参两栏**（**09-13 减法后**——旧版是「图 + 框架 + agent 三合一」） | `prototype/知所栖-壳.html` | **09-13 03:1x 按所有者指令做减法**（原话：「只保留内参+知识体系。知识体系直接按主题分，也就是把那个策展的功能给取代掉了。然后『我在学』和『对话』，还有下面这个栏，该做减法的就做减法，因为到时候要去交给我的队友去做另外的一些功能」）。**tab 板（图谱 / 星球）09-13 提到三栏之上**：横跨列表栏与主区、位于「知识体系」那一行**上方**，导航栏跨两行一路到底（所有者要求「内层的 tab 板要在更上方，优于知识体系」）。**左栏＝品牌 + 搜索框 + 两个导航项 + 主题分类列表**（21 条；样式照所有者给的 candobear BoK 参考图：分组标题「主题」+ 右侧「全选」，每行 8px 彩点 + 名称 + 右对齐数字；点一条＝只看这一列、再点取消）——追加① 曾把这块整片删掉，追加③ 按参考图做回来。09-13 第二轮补删又去掉了左栏的「主题 · 点击筛选」标题、21 条主题图例、底部 `0 0 0 ／936` 状态点（所有者：「都不要了」）；**画布上仍按 21 个主题分列**（`groups()` / `giOf()` 没动），只是不再在左栏列第二遍。**底部 `import`/`insert`/`Agent` 那条栏整条删除**；横轴**固定按主题分列**（21 条线），旧的「怎么验 / 按来源」两档与 `setAxis` 一起撤掉——验收方式仍逐张写在概念卡上（`KIND`），只是不再当分列轴。**删掉的四个视图**（策展 / 待你看一眼 / 我在学 / 对话）连同代码路径、CSS、`/api/skills` 与 `/api/llm` 的前端调用方一起删；`localStorage` 里的 `marks`（费曼验收记录）保留，概念面板的倒逼判定照旧。模板 1865 → 1160 行，壳 1.20 MB → 1.16 MB（大头是 936 概念的 payload，不是代码）。Linear 式三栏结构不变（导航 224 / 列表 302 / 主区 / 详情 412 停靠让位）。**「内参」一栏**：列表＝当期文章（主题 + ★），主区 `#reader` 是阅读页（三级笔记 / 概念网络 / 费曼 ×3 / 阅读原文四个分页签），数据由 `scripts/build-neican.mjs` 生成。概念面板的「深看」折叠块（掌握证据 / 验收问句 / 学习时机）不变；**「费曼一下 / 原文 context / 别名」09-13 15:5x 拆门**——原来锁在「过了才给」后面（判过才渲染），现在 936 张卡随时可看，展开**不写任何验收状态**（倒逼状态机没动）；**「来源」由死标签改成可点**（`cm-wire.mjs` 出 `articles` 表：294 条 ＝ 65 篇原文 ＋ 229 个 Notion 页，节点用 `sa` 指过来；卡上列前 6 篇 ＋ 每篇的「同源概念」）。同轮修掉两个真 bug：`sourceContext`（919/936）与 `aliases`（414/936）**一直没被搬进 payload**，卡上那两栏是空的；`assessmentPrompt` 里 **922/936 条**留着未替换的 `{{name}}` 占位符（改在 `cm-wire` 出线前替换；地图 JSON 里仍在）。源：`scripts/shell.template.html` ＋ `scripts/build-shell.mjs`。**09-13 19:2x 加「路径」视图**：主区顶部变成 `路径｜图谱｜关系｜星球`，**路径是默认**；左上 262px 路径条＝方案 B 的导师（第 N 步/共 6 步 · 为什么现在学 · 前置 · 下一步 · 卡住回退 · 分支 ≤2 · 回到全图），画布上路线 6 点标号加亮、当前步白环、其余变灰缩小（仍可点）；概念卡顶部加路径上下文；主题下钻后顶部出现路径入口。**必须经服务打开**：`node scripts/serve-135.mjs` → `http://127.0.0.1:5180/知所栖-壳.html`。**09-13 减法后验收全重跑通过**：`shot-shell.mjs`（13 步 + 0 JS 报错）· `test-daobi.mjs`（29 项）· `check-public.mjs`（21 项）· `test-app.mjs`（12 项）。**09-13 15:5x 追加后重跑**：`test-daobi.mjs` **40 项全过** · `shot-shell.mjs` 0 JS 报错 · `check-public.mjs` 全过（壳 1.39 → 1.76 MB，增的是 sources/context/别名，不是代码） |
