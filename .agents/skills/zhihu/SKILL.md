...
name: zhihu
description: >-
  使用知乎开放平台搜索知乎和全网内容、获取热榜、调用知乎直答、发现适合回答的问题、查看问题下的回答摘要，读取本人创作全文、评论、账号与单篇创作数据，以及关注与收藏，列出、检索和上传知识库，或查询开放 API 剩余额度。用户提到知乎搜索、社区观点、真实经验、热点、热榜、知乎直答、问题推荐、选题、回答摘要、我的知乎内容、我的关注、我的收藏、知识库、RAG、API 额度、剩余额度、用量、开放平台、API、MCP、Access Secret，或要求查看、安装和配置知乎 Skill 时使用。深度研究优先返回搜索原始来源；本人数据和知识库只读取完成任务所需的最小范围。
---

# 知乎开放平台

## 本项目安装约束（优先于下文默认安装与认证步骤）

用户要求仅在当前项目使用。官方包版本为 0.6.0；此副本增加项目隔离适配。
- 所有 CLI 调用使用项目根目录的 `scripts/zhihu`（可从任意工作目录以绝对路径调用）。
- CLI 二进制只安装在项目 `.local/zhihu-cli/`；不修改全局 PATH 或全局 Skill。
- 凭证只保存在项目 `.private/zhihu/access-secret`，权限 600，目录权限 700；由启动器仅注入子进程 `ZHIHU_ACCESS_SECRET`。
- 不执行 `auth set`、`auth logout` 或 `init`，不写系统钥匙串，不回显或提交凭证。缺少项目凭证时停止，不回退其他项目或系统凭证。
- 每个 Session 首次检查使用 `scripts/zhihu status --skill-version 0.6.0 --min-cli-version 0.6.0`。
- 初始化验收用 `scripts/zhihu auth status --verify` 及 `scripts/zhihu me contents --type all --limit 1`；只报告状态，不保存本人内容。
- 不将 `.private/`、`.local/` 打包上传或部署。
