...
> 把模型、system prompt、工具、MCP servers、skills 存成 YAML 放进 git，由 CLI 在流水线里 apply。

**领域** state-persistence ｜ **类型** PROCEDURAL ｜ **什么时候学** 做到这里再懂 ｜ **怎么算会了** 能用 ｜ **中心度** 0.099
...
- 能写出一份可被 CLI apply 的 agent 模板 YAML
- 能说明 CLI 做 setup、SDK 做 runtime 的分工

## 验收问句

> 如何把 {{name}} 落地到 git 与部署流水线里？

## 先懂这些（前置 3）

- [[Git-backed state]] · **hard** — 把模型、prompt、工具等存成 YAML 放进 git，需先懂 git 持久状态。
- [[文件系统即持久记忆]] · **soft** — 声明式配置以文件承载，懂文件系统持久记忆有帮助。
- [[文件系统即持久记忆]] · **soft** — 模板以 YAML 存进 git 文件并由 CLI apply，先懂状态以文件持久才懂声明式持久化。
