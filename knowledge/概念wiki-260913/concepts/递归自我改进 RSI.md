...
不是「机器变聪明」，而是「机器改进让自己变聪明的那条生产线」。你不是自己长脑子，而是把你的学校、教材和考试制度改得更好，于是下一届的你更强。

## 原文 context

全文的母题与线索起点。I. J. Good（1965）定义「ultraintelligent machine」为能在一切智力活动上超过人类、并设计出更好机器来改进自身的系统；Yudkowsky（2008）把它收窄成一个具体反馈回路：AI 用当下的智能去改进那套产生它智能的认知机器。文章明确指出，现代形态未必是模型改写权重，更广义是模型改进训练流水线与部署系统。

## 掌握证据（做到这些才算会）

- 能说出 Good 1965 与 Yudkowsky 2008 的定义演进
- 能指出改写权重只是其中一种形态
