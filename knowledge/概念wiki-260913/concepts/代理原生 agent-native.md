...
> 不是给旧产品外挂 chatbot，而是从底层把产品改造成供 agent 使用、并为 agent 提供上下文与集成的形态。

**领域** agent-org ｜ **类型** CONCEPTUAL ｜ **什么时候学** 做到这里再懂 ｜ **怎么算会了** 能判 ｜ **中心度** 0.126

## 费曼一下

普通软件是给人用的，界面、按钮、说明都围着人转。agent-native 软件是给 AI 用的——它默认读者是一个 agent，于是把“该带什么上下文、能调哪些能力、怎么完成任务”都摆好。就像同一栋楼，一种是为行人设计的商场，另一种是为无人车设计的仓库：后者的通道、标识、货架都按机器的读取方式重排过。

## 原文 context

文中反复出现的核心定位词。Linear 被描述为“成功转型为 agent-native”的少数公司之一——不是给旧产品外挂一个 chatbot，而是把产品从底层改造成“供 agent 使用、并为 agent 提供上下文与集成”的形态。它的 agent 平台文档很好，agent 能自己读文档、自建集成，因此大多数 coding agent（含 OpenAI 的 codex）都接了进来。

## 掌握证据（做到这些才算会）

- 能区分外挂 chatbot 与底层 agent 化改造
- 能说明文档可自读、集成可自建为何是 agent 接入的前提

## 验收问句

> {{name}} 与加个 chatbot 的本质差别是什么？

## 先懂这些（前置 1）

- [[二八反转]] · **soft** — 八成交互经 agent 完成后，产品才必须从底层为 agent 改造。

## 懂了它才能懂（解锁 3）

- [[并不 agentic」的 AI Agent]] — 要识别产品是否真 agentic，先得理解 agent 原生是什么形态。
- [[共享产品系统 shared product system]] — 人与 agent 共同工作的系统，前提是产品已为 agent 改造并提供上下文。
- [[小而模块化的概念]] — 要从 agent 构建中取概念嵌入产品，先得理解 agent 原生形态。
