...
**agent 是台前**——用户看到的那个会干活的智能体；**harness 是台后**——让那个智能体能干活的那套机器。说 "我做了个 agent" 本质是 "我造了个 harness，把它对准了一个模型"。

## 原文 context

The "agent" is the emergent behavior: the goal-directed, tool-using, self-correcting entity the user interacts with. The harness is the machinery producing that behavior.
