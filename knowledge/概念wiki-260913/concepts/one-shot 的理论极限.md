---
id: cm_1536f249
name: one-shot 的理论极限
type: CONCEPTUAL
subject: Harness Engineering
domain: context-engineering
learningStage: when-needed
verification: accept
centrality: 0.017
depth: 0
origin: [harness]
aliases: []
sources: 1
---

# one-shot 的理论极限

> 理论上提示、上下文、顺序都完美就能一次做对，但没人做得到，规划的意义正是逼近它。

**领域** context-engineering ｜ **类型** CONCEPTUAL ｜ **什么时候学** 做到这里再懂 ｜ **怎么算会了** 只能认 ｜ **中心度** 0.017

## 费曼一下

完美提示词是个存在但够不着的点。承认够不着，就会把力气从「找咒语」转到「搭流程」上——后者才是可复制的。

## 原文 context

作者的清醒判断——「In theory, you can get a model to one-shot anything if you compose the perfect prompt with the perfect context in the perfect order. In theory.」但没人做得到，规划的意义正是让你逼近这个理想。

## 掌握证据（做到这些才算会）

- 能复述'In theory'这一限定并指出其不可达性
- 能解释规划为何是逼近该理想的手段

## 验收问句

> 为什么 {{name}} 存在却仍必须做任务规划？

## 相关

- [[prompt 主导论]] · rejected（audit） — one-shot 极限只要求提示/上下文/顺序可完美，不依赖“prompt 比 harness/模型更重要”的排序；只是相关。
- [[「少即是多」：gimmick 与真实增益的分界]] · 同篇出现（co-occurrence） — 同篇出现：harness-13
- [[废料怪兽]] · 同篇出现（co-occurrence） — 同篇出现：harness-13
- [[Harness]] · 同篇出现（co-occurrence） — 同篇出现：harness-13

## 出场

- Harness Engineering ｜ 《GitHub：决定 AI 编码效果的是 harness，不是你换了哪个工具》 ｜ https://github.blog/company/the-harness-is-all-you-need-mostly/
## 反链

- [[废料怪兽]]
- [[prompt 主导论]]
- [[「少即是多」：gimmick 与真实增益的分界]]
