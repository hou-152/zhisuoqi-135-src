...
AI 不再是你正在聊天的同事，而像代码里的一个函数。你的程序需要判断时调用它，用完再根据返回结果继续执行。

## 原文 context

作者说 loop 出现后，模型变成系统里的 subroutine。人类不再把 Claude 当成一个对话对象，而是把它当成 loop 在某一步调用的能力。

## 掌握证据（做到这些才算会）

- 能指出自己循环中调用模型的那一步
- 能对比对话式用法与 subroutine 式用法的差别
