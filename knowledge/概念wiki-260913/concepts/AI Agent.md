...
AI Agent 是用户看到的“会做事的东西”。它看起来有目标、会用工具、会纠错，但这种行为不是裸模型单独产生的，而是 Harness 把模型组织进执行系统之后的外在表现。
...
- 能区分 Agent 的内部实现与用户感知行为
- 能举出一个用户可观察的 Agent 行为

## 验收问句

> 从外部看，我们凭什么判断 {{name}} 是否符合预期？
