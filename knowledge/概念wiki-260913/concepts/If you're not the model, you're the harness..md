---
id: cm_67483624
name: "If you're not the model, you're the harness."
type: CONCEPTUAL
...
agentic AI 世界里，万物二分。你做的要么是训练模型，要么是做 harness。**没有第三条路**。

## 原文 context

LangChain Vivek Trivedy 的断言。

## 掌握证据（做到这些才算会）

- 能对任一 agent 系统把组件分成模型侧与 harness 侧
- 在争论某能力归属时能引用这条划界

## 验收问句

> 按 {{name}}，这个功能算模型还是算 harness？

## 懂了它才能懂（解锁 2）

- [[Harness 工程 Harness Engineering]] — 不懂这条划界公式，就说不清 Harness 工程的范围、也不知道除模型外该由谁负责
- [[非模型架构 Non-model Architecture]] — 不懂划界公式，就列不全非模型架构到底该包含哪几层成分
...
- [[Harness]] · related-to（audit） — 口号本身即以排除法界定 harness，不懂 Harness 也能读懂划界，非 hard 前置。
