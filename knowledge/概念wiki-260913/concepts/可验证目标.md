...
**领域** verification-eval ｜ **类型** CONCEPTUAL ｜ **什么时候学** 现在先懂 ｜ **怎么算会了** 能判 ｜ **中心度** 0.135
...
- [[结构断言与不变量断言]] — 断言阈值取决于目标能否被机器判定，不懂可验证目标就选不准结构与不变量。
- [[ALFRED]] — ALFRED 指令用清晰显式指代，正因目标须机器可判定，不懂此点读不懂其设计。
- [[back-pressure]] — 自我验证要成立，前提是目标能被机器判定，否则成功与否无从衡量。
