---
id: cm_f0aa9b79
name: 纠正成本递减
nameEn: correct things less from that point on
type: CONCEPTUAL
subject: Context Engineering
domain: harness-runtime
learningStage: when-needed
verification: judge
centrality: 0.017
depth: 0
origin: [context]
aliases: ["correct things less from that point on"]
sources: 1
---

# 纠正成本递减 · correct things less from that point on

> 改进 harness 的收益不只在当轮，而是从该轮起持续减少后续需要人工纠偏的次数。

**领域** harness-runtime ｜ **类型** CONCEPTUAL ｜ **什么时候学** 做到这里再懂 ｜ **怎么算会了** 能判 ｜ **中心度** 0.017

## 费曼一下

前面多花十分钟说清楚，后面就少了几十次“不对，我不是这个意思”。

## 原文 context

可观测的结果是“have to correct things less from that point on”——收益不只在当轮，而是从这一轮起持续降低后续的纠偏次数。

## 掌握证据（做到这些才算会）

- 能记录改进前后一段时间内人工介入次数并对比
- 能区分一次性修复与产生持续复利的改进

## 验收问句

> 改进 harness 后，你如何用后续纠偏次数验证{{name}}？

## 相关

- [[漫谈会话 ramble session]] · 同篇出现（co-occurrence） — 同篇出现：context-09
- [[bits]] · 同篇出现（co-occurrence） — 同篇出现：context-09
- [[输入摩擦 too lazy to type]] · 同篇出现（co-occurrence） — 同篇出现：context-09

## 出场

- Context Engineering ｜ 《用十分钟语音漫谈，让 LLM 帮你整理真正想说的话》 ｜ https://x.com/karpathy/status/2079610838143623371/

## 别名

`correct things less from that point on`

## 反链

- [[bits]]
- [[漫谈会话 ramble session]]
- [[输入摩擦 too lazy to type]]
