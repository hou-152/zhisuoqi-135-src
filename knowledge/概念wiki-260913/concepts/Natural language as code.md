...
Natural language as code 指自然语言不再只是说明文字，而是会驱动 agent 行为的可执行逻辑。

## 掌握证据（做到这些才算会）

- 能把一句说明改写成会改变 agent 行为的指令
- 能判断某段自然语言是否真的会被执行

## 验收问句

> 你这段自然语言会真的改变 {{name}} 的行为吗？

## 懂了它才能懂（解锁 1）

- [[从禁止什么到对齐什么]] — 不懂【Natural language as code】，就做不了【从"禁止什么"到"对齐什么"】里「把一条对齐指令写成能驱动 agent 行为的逻辑」

## 相关

- [[Instructions files]] · related-to（audit） — 指令文件是「自然语言即可执行逻辑」的一种形态/例子，不是其前提；该概念同样可由 prompt、skill 等其他形态支撑，属举例而非依赖。
