...
> 默认前提：模型处理第10000个token应与第100个一样可靠；报告以实验一致地证伪了它。
...
- 能指出该假设成立时应有的预测：只加长输入、难度不变则成绩不变
- 能用实测结果说明成绩随输入长度一致变差

## 验收问句

> {{name}}若成立，只加长输入而任务不变，成绩该怎样？

## 懂了它才能懂（解锁 2）

- [[性能梯度而非硬悬崖 performance gradient rather than a hard cliff]] — 梯度论正是对均匀处理假设的否定，先懂假设才懂它在反驳什么
- [[长上下文的幻觉]] — 它同样在否证「窗口更长就均质可用」的默认心证。
