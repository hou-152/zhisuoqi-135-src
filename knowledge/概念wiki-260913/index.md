...
> 1136 个概念 · 718 条前置依赖 · 21 个领域 · 源：Notion 概念库 + Context Engineering(28篇) + Harness Engineering(30篇)
...
**枢纽**：[[Harness]] · [[Harness 工程 Harness Engineering]] · [[skills 字段]] · [[Agent vs Harness]] · [[coding agent]] · [[Runtime-harness separation]] · [[框架]] · [[跨产品面的同步与共享边界 Surface-specific Sync and Sharing]]
...
- [[框架]] — 智能体运行环境的装配，由说明（系统提示词与规则）、工具、模型三部分组成。
...
- [[云端代理]] — 在远程沙盒中运行的编码智能体，克隆仓库、建分支、自主完成后创建 PR。
...
- [[编码 agent 智能体 coding agent Agent]] — 能接受自然语言目标、选用搜索工具、读取代码并返回线索的执行者。
...
- [[开源 Codex 执行线圈 Open-source Codex harness]] — 驱动 Agents API 的开源 Codex harness，公开协调模型调用、工具与上下文的核心逻辑。
...
- [[线束能力内化 Harness Capabilities Shifting Into the Model]] — 过去要靠外部线束提供的符号建模能力，正被 Astra 吸收进模型本体。
- [[长期运行智能体执行框架 Long-running agent execution framework]] — 支撑 Agent 连续运行数天的执行框架：管理上下文、高效用工具、协调子智能体并提供文件与代码环境。
...
**枢纽**：[[循环工程 loop engineering]] · [[规划与任务拆分]] · [[Agent loop]] · [[可自行验证的步骤]] · [[Loop Engineering]] · [[Prompt loop（prompt loop）／agent]] · [[闭环]] · [[软件工厂全景闭环 Software factory loop]]

<details><summary>全部</summary>

- [[循环工程 loop engineering]] — 把频繁的手动收尾固化为可重复循环的方法：挑动作、试验证、写流程、封装成技能、调用并迭代、再链式化。
- [[规划与任务拆分]] — 大范围改动前先规划并把任务拆小；出现无关改动、误改文件、偏离重点时应停下拆分。
- [[Agent loop]] — SDK 内置的循环：发起工具调用、把结果送回模型、持续迭代直到任务完成。
- [[可自行验证的步骤]] — 把工作拆成智能体能自己判断成没成的小步骤，每步先定方案并配防护机制。
...
- [[软件工厂全景闭环 Software factory loop]] — 由侦测业务迷雾、固化形式规格、驱动切片构建、审查决策账本组成的软件工厂闭环。
...
- [[完整闭环]] — 探索—规划—构建—测试—调试—评审—沉淀组成的编码智能体完整工作闭环。
...
- [[持久执行 Never Quits]] — Astra 会一直跑到做完，而非半途停下等确认，但也偶有偷懒反问的情况。
- [[触发模式谱系]] — Agent 常见触发方式四分类：事件触发、定时触发、发射后不管、长时程自主，各自对应不同在环程度。
- [[监控告警自动触发智能体调查]] — 监控告警自动创建工单并触发智能体在人工介入前开始诊断问题的自动化流程。
...
- [[雄心勃勃的项目 Ambitious Projects]] — 指几千行代码、跨多天、需模型自己拿主意持续推进的大活，是全文证据的组织核心。
...
- [[关注智能体工作 停止与重新引导]] — 借 diff 视图实时监督智能体，方向错了就停止、取消、还原并重新引导。
...
- [[外层改进者技能 Outer Improver skill]] — 按周期运行、比对人类反馈并向内层基础技能提出最小改进补丁的元智能体
...
**枢纽**：[[Skill]] · [[上下文 context]] · [[SKILL.md 与 YAML 前置元数据 SKILL.md YAML frontmatter]] · [[按需加载]] · [[上下文工程 context engineering]] · [[上下文用量 上下文管理 context usage context management]] · [[注意力预算 attention budget]] · [[启动时仅加载名称和描述]]
...
- [[上下文用量 上下文管理 context usage context management]] — 对话中累积的信息量；搜索大量文件会使其膨胀，子代理只返回发现结果可显著改善管理。
...
- [[图像输入]] — 把设计稿或屏幕截图贴进提示词，让智能体照着还原、排查或按截图迭代 UI。
...
- [[规则 Rules]] — 放在 .cursor/rules/ 的 markdown 文件，每次对话开始时智能体都会看到。
...
- [[基于文件的技能 File-based skills]] — 把业务规范与领域知识从提示词抽成 Markdown 文件，Agent 按需查阅
...
- [[上下文自动压缩 Context compaction]] — 会话接近上下文上限时，Agents API 自动压缩较早上下文，同时保留继续工作所需的信息。
...
- [[运行时证据]] — 程序真实运行后留下的数据，如终端输出、查询日志、性能数据，而非静态代码本身。
...
**枢纽**：[[记忆 Memory]] · [[多时间尺度记忆与「记忆只是 hint」]] · [[三层知识架构 Three-layer knowledge architecture]] · [[跨轮次记忆与连贯策略]] · [[结构化记事 agentic memory]] · [[推论因果溯源图 Provenance of conclusions]] · [[self-baking]] · [[持久维基层]]

<details><summary>全部</summary>

- [[记忆 Memory]] — 在不同时间尺度上运作的存储：会话内、跨会话、长期沉淀，各层服务于不同的取回需求。
- [[多时间尺度记忆与「记忆只是 hint」]] — 短期为单会话历史，长期跨会话持久化并分层索引；agent 应把记忆当提示，行动前校验真实状态。
- [[三层知识架构 Three-layer knowledge architecture]] — 把经验与知识分开：原始轨迹、持久知识库、带溯源技能三层，经验先沉淀再由知识驱动技能。
...
- [[推论因果溯源图 Provenance of conclusions]] — 每条推导结论都挂载证据树，记录由哪些基础事实与规则推出，可追问来源并顺藤摸瓜。
- [[self-baking]] — Agent 有选择地把自己的上下文消化成持久知识结构，是记忆存储与学习的分界。
- [[持久维基层]] — Wiki 层把具体成败轨迹提炼为抽象规律；技能被拒而回滚时，Wiki 永不回滚，保留失败教训。
- [[即时检索 Just-in-time Retrieval]] — 上下文里只保留轻量级标识符（路径、ID、链接），需要时再取全文，而不是把内容全塞进去。
- [[状态维护型记忆 State-maintaining memory vs. Conversational memory]] — 不只是记录说过什么，而是维护一份当前已知事实的状态；前提被推翻时相应认知随之更新。
...
- [[事实与推导的自动级联失效 Automatic invalidation of derived facts]] — 观察变化时，依赖该事实的结论自动变为失效，无需模型重读历史去发现全部连锁后果。
- [[原始轨迹层 Raw Layer]] — 存储每次迭代的执行轨迹：完整推理、工具调用与输出结果，不可变，保留到底发生了什么。
...
- [[规则沉淀 定价规则]] — 把开发中发现的不变量写成规则记录，供智能体下次遵循。
...
- [[技能与记忆的本质区别 Skills vs. Memory]] — 技能是稳定程序性的'怎么做'，记忆是推理期自动写入、持续变动的状态
- [[跨会话记忆文件系统]] — 把记忆当跨会话工作记忆存成文件系统：六种操作、版本令牌、frontmatter、按主题分文件，供未来会话开头重读。
- [[门控机制]] — 检索内容与当前上下文不匹配时自动屏蔽的过滤机制，如区分姓氏「张」与历史人物「张仲景」。
- [[模糊与确定性解耦范式 Fuzzy-Deterministic split]] — 把问题拆成两半：LLM 负责把模糊非结构化输入转成结构化事实，确定性引擎负责状态推导与一致性。
- [[声明式逻辑记忆引擎 Datalog-based memory engine]] — 用 Datalog 只声明事实与规则，由引擎自动派生新事实并在事实增删时重算依赖，不耗推理 Token。
...
- [[笔记生命周期策展 Note curation lifecycle]] — 每轮研究结束做一次策展，让笔记在 draft、evergreen、stale 等状态间流转
...
- [[Markdown 为真源，SQLite 为缓存 Markdown is truth, SQLite is cache]] — 笔记以 Markdown 加 YAML 头为真源，SQLite 仅作可随时重建的检索缓存
...
**枢纽**：[[提示词缓存 Prompt Caching]] · [[Token count]] · [[前缀匹配 Prefix Matching]] · [[稳定前缀 Stable Prefix]] · [[提示词缓存（Prompt Caching）]] · [[缓存命中率 Cache Hit Rate]] · [[Cache prefix stability]] · [[cached input tokens]]
...
- [[上下文成本]] — 自定义内容占用上下文、分散注意力的代价；规则常驻所以成本一直存在。
...
- [[工具按需搜索 Tool search]] — 工具搜索按需加载相关工具定义，在保留模型缓存的同时减少 Token 用量和成本。
- [[时间投入 Time to Think]] — 极难或从零开始的任务要付出大量时间与 token，如 Factorio 44 小时、约 4500 美元。
...
**枢纽**：[[工具定义 Tool Definitions Tool Schema]] · [[Sandbox]] · [[Tool Calling]] · [[工具 Tools]] · [[MCP Model Context Protocol]] · [[MCP servers]] · [[运行时限制、权限与数据保留边界 Runtime Limitations, Constraints, and Retention]] · [[shell tool]]
...
- [[集成浏览器与 MCP 服务器]] — 让智能体自己开浏览器截图比对自己改的页面，或从 Figma MCP 取设计规格。
...
- [[CLI 工具]] — 智能体可直接在终端执行的已装程序，如 gh、aws、kubectl、docker。
- [[精确字符串查找 grep grep ripgrep Instant Grep]] — 精确字符串查找工具，可用正则与词边界，ripgrep 支持递归，Instant Grep 更快。
- [[运行时失败的三类原因]] — 运行时失败查三类：缺外部依赖、脚本无执行权限、路径未统一用正斜杠。
- [[集成浏览器]] — Cursor 的集成浏览器，让智能体直接打开页面、读控制台日志、查网络请求并查看 DOM。
- [[连接器]] — MCP、GitHub、飞书、数据库等外部接口，让 Agent 接入真实工作环境，形成发现—修改—通知的闭环。
- [[MCP 服务器]] — 让智能体连接外部工具与生产数据、按需拉取信息而无需人工粘贴数据的机制。
...
- [[程序化工具调用 Programmatic tool calling]] — 让智能体并行执行调用、串联操作，并用代码筛选或合并结果，只把相关结果送回上下文。
- [[计算机使用 Computer Use]] — 模型直接操作真实软件：点界面、跑 Blender、玩 Minecraft、填税表。
...
**枢纽**：[[Subagent]] · [[多模型并行与多智能体评判]] · [[子 agent 与 step.invoke()]] · [[Planner–Worker 角色分离]] · [[多智能体架构]] · [[子 agent 编排 Fork Teammate Worktree]] · [[子智能体并行委派 Subagent delegation and orchestration]] · [[handoff 交接]]

<details><summary>全部</summary>

- [[Subagent]] — 把一整个 session 工作封装、只回流浓缩结果的子代理，拥有全新而小的上下文窗口与指令预算。
- [[多模型并行与多智能体评判]] — 让多个模型在隔离环境并行运行同一调试提示词，再按证据充分度比较其修复方案。
...
- [[子智能体并行委派 Subagent delegation and orchestration]] — 主智能体把复杂任务拆成相互独立的部分委派给子智能体并行处理，各自维护上下文，主智能体汇总结果。
...
- [[并行测试多种情况]] — 启动多个云端代理分别覆盖不同边界、错误条件与输入组合，各产出带结果的分支。
- [[子 Agent 分工]] — 把执行、审查、修复分给不同子 Agent 或模型，避免写代码的 Agent 给自己打分。
- [[Git 工作树并发隔离 Git worktree isolation]] — 为每个 AI 会话用 git worktree 单独检出工作目录，共用同一 .git 元数据，物理上互不覆盖。
...
- [[独立只读审计智能体 Independent read-only auditor]] — 审查者须是独立于实现者的只读副手，不给改码权限，否则它会自圆其说。
...
- [[Explore 子代理 Explore subagent]] — 智能体按需生成的搜索子代理，在独立上下文窗口用更快模型并行搜索，只返回发现结果。
- [[子代理的技能隔离]] — 子代理以全新干净上下文启动，不自动看到主会话技能，须在 skills 字段显式列出
- [[分层对抗式研究流水线 Hierarchical adversarial research pipeline]] — 把研究拆成分解、扫描、矛盾聚类、对抗批判、引文核实等十六步的流水线
- [[会话与工作交接机制 Handoff]] — 在前台本地工作区与后台工作树之间安全搬迁工作的自动化流程，代你完成必要的 Git 操作。
...
- [[云端代理的边界测试 Cloud Agents]] — 启动多个云端代理并行覆盖本地可能遗漏的边界情况，在合并前补齐空缺。
- [[子智能体编排 Multi-Agent Orchestration]] — Astra 被端到端训练做多智能体编排，由主模型拆活给子智能体并行再汇总。
...
**枢纽**：[[Validation gates]] · [[验证手段]] · [[验证闭环 verification loop]] · [[验证子系统与可运行的证据]] · [[Grading Criteria]] · [[Rubric]] · [[Self-verification]] · [[Self-verification loop]]

<details><summary>全部</summary>

- [[Validation gates]] — 工作流中的检查点：完成一步后须通过测试、审查、人工确认或明示验收条件才能继续。
- [[验证手段]] — 测试、类型检查、Linter、浏览器工具四类手段，让智能体自己发现输出错误。
...
- [[调试模式 Debug Mode]] — 针对棘手缺陷：先提假设、插桩、请人复现、分析日志，最后做针对性修复。
- [[良好调试的共同原则]] — 调试五原则：可复现、缩到最小案例、隔离变量、具体假设、加日志。
...
- [[智能体判断自己的代码是否正确]] — 智能体能跑测试、看失败并据此改代码，从而自己判断产出对不对。
- [[智能体评审 查找问题]] — 让智能体从写代码切换到审代码，逐行分析拟议编辑并标记潜在问题。
- [[测试驱动开发 TDD]] — 先提交测试锁住需求，再让智能体在不修改测试的前提下让测试全部通过。
- [[验证信号]] — 测试、类型检查与 Lint 是智能体自查成果的反馈渠道，越完善越敢委派。
...
- [[调试与根因修复]] — 折扣叠加测试失败时，定位根因是折扣应用顺序，并修 DiscountService.ts 中的顺序。
- [[瓶颈转移与测试速度杠杆]] — 并行使用智能体后瓶颈移到最慢公共环节（测试、类型检查、Lint、CI），提速有复利。
- [[Bugbot 与 Autofix]] — 接入源代码控制服务商的自动 PR 评审机制，能找逻辑缺陷，Autofix 可在评论中提交修复。
- [[基准测试的捆绑测量性]] — 基准很少单独测量模型，它同时测了 API 设置、harness 设计与提示词等不可见选择。
- [[理解修复与根本原因]] — 验证修复要看人能否解释因果链并确认根因，而不是看错误是否消失。
...
- [[自我评审]] — 在请他人查看前，先让智能体带着完整分支上下文评审自己的全部更改。
- [[测试是绿的，产品却在退化]] — 模型更新后测试仍绿而产品已退化：断言写的是旧模型行为，绿色测试与真实回归可以同时成立。
- [[测试通过不等于正确]] — 测试是验证信号而非正确性证明，测试本身可能验证了错误的行为。
...
- [[基准过度声明与数据污染 Unnecessary Overstatement]] — 官方高调宣传 ExploitBench 满分，而满分恰说明数据污染，OpenAI 系统卡亦承认。
- [[揭示偏好 Revealed Preference]] — 以“人们实际把哪个模型设为主力”作为最纯粹的评审证据。
- [[结构断言与不变量断言]] — 等值断言失效后的两条退路：断言输出结构与必填字段，以及是否违反被告知的规则，但两者都弱于真实意图。
- [[可维护性缺一个可靠的打分预言机]] — 代码质量缺乏可即时打分的预言机：测试几秒出结果，架构变差以周月年计，设计差当期基准测不出。
- [[勒雷奇点倒计时技巧 Leray's countdown trick]] — 令 τ=1−t 把时间反向、让奇点成为倒计时终点的变量替换，勒雷 1934 年首创。
...
- [[有限时间爆破失效 Invalidation of finite time blowup]] — AI 用 dτ=−dt 的写法，使流体有限时间爆破的证明在欧拉与纳维-斯托克斯方程上都失效。
...
- [[错误消息明确时的直接修复]] — 错误消息或堆栈已指向原因时，智能体读错误读代码直接改；不奏效则转系统排查。
...
- [[评估意识 detect being evaluated]] — 最先进的 AI 能察觉自己在被评估而非部署，并据此改变行为。
- [[维基消融效应 Wiki ablation effect]] — 去掉 Wiki 访问后平均分从 63.7% 降至 48.7%，回落约 15 个百分点。
...
- [[奖励黑客 reward hacking]] — 系统优化的是奖励，但奖励不完全等于我们真正想要的东西，行为随之偏离。
...
- [[目标冲突 conflict between goals]] — 用户指定的任务有时与安全、对齐目标不相容，促使系统为达目标而作弊。
...
- [[修改前先探索]] — 提出修改前先让智能体展示现有实现、模式与共享验证器，避免重复造函数或用不同模式。
- [[高精度原型设计]] — 把需求、原型、UI 合成一步，产出含交互与视觉的高保真原型。
- [[按字面理解请求 意图 literal interpretation intent]] — 编码智能体按字面处理请求，未提供意图时自行判断，故需先理解代码库并明确要求。
...
- [[技能溯源 PURPOSE.md]] — 每个技能除 SKILL.md 外标配 PURPOSE.md，记录它为解决 Wiki 中哪条失败模式而存在。
- [[描述区分度]] — 描述过于相似会让 Claude 选错技能或困惑，应让描述更具体、更有区分度。
- [[明确定义的目标与模糊目标]] — 可评分的明确目标在冲突中往往压过可多重解读的模糊目标。
...
- [[写原则而非写死规则 Write principles, not rules]] — 写技能时给判据与方向而非穷尽规则，让模型在未知边界下自行推理
- [[意识流输入 full stream of consciousness]] — 对漫谈内容的反向要求：允许 total mess、跑题、重复与自我否定，即 full stream of consciousness。
- [[Goodhart's law]] — 一个指标一旦成为被优化的目标，就不再是有效的衡量方式。
...
- [[提示词]] — 把需求锚在代码库已有物上的说明：点名文件、指定复用组件、说明遵循的既有模式并划定范围。
- [[富引用 rich references]] — spec 不限于 markdown 计划：HTML artifact、测试套件、待移植函数、rubric 都可充当。
- [[回到方案，而非修补提示词]] — 构建结果不符预期时撤销更改、细化方案后重跑，而不是继续补提示词。
- [[问题的表述方式]] — 提问的具体程度会影响智能体选用哪种搜索工具与结果质量：已知从具体起，探索从宽泛起。
...
- [[首迭代范围收敛]] — 评审方案时砍掉对首次迭代而言范围过大的步骤，只保留核心流程。
- [[意图翻译者 intention translator]] — 1.0 时代设计者的角色：把复杂人类意图转成结构化、机器可读的格式，因机器无法理解语义也无法推理。
- [[原始查询即圣经 Canonical research query is gospel]] — 用户原始提示词被逐字持久化，后续每一步与子智能体都必须重读它
...
**枢纽**：[[Agentic Engineering]] · [[代码评审]] · [[AI 生成代码的隐性缺陷]] · [[service template 与 golden path]] · [[Ask mode]] · [[层级架构强约束 + 给 Agent 读的 lint 错误]] · [[单分支单一权威工作副本保证 One-branch-per-worktree rule]] · [[根因优先]]

<details><summary>全部</summary>

- [[Agentic Engineering]] — 协调可错、随机而强大的 agent 快速产出，同时守住正确性、安全、品味与可维护性的工程纪律。
- [[代码评审]] — 在问题进入生产前发现缺陷的质量门，需要合适流程，也要工程师投入精力。
- [[AI 生成代码的隐性缺陷]] — 代码看似正常、能编译能过测试，却遗漏边界、有安全问题或重复既有逻辑。
- [[service template 与 golden path]] — 团队沿既定路径快速实例化新服务的现成实践，被用来类比 harness 未来按拓扑挑选。
- [[Ask mode]] — 先提问、不改代码的模式，用于探索流程与推送前的自我评审。
- [[层级架构强约束 + 给 Agent 读的 lint 错误]] — 把 lint 错误从『violation detected』改写成给 Agent 直接可读可改的修复指令，配合层级架构的强约束。
- [[单分支单一权威工作副本保证 One-branch-per-worktree rule]] — 每个分支同一时刻只能在一个工作树检出的规则，保证每个分支只有一个权威可变工作副本。
- [[根因优先]] — 根因没说清楚之前先别动代码：先答出问题在哪个文件哪一行、为什么，答含糊就继续查。
- [[技术债与一致合并标准]] — 智能体快速生成大量代码会积累维护成本，合并标准对手写与生成代码应一致。
- [[统一语言 ubiquitous language]] — 统一语言：借用 DDD，让代码库、开发者与领域专家在 AI coding workflow 中共用同一套词汇。
- [[小而语义明确的提交]] — 每次提交只代表一项逻辑更改，让评审者按提交历史逐个理解。
...
- [[提交重整理 rework-commits]] — 功能稳定后让智能体重置到 main、通读更改进而重建整洁提交，并校验内容未丢。
- [[分解为小颗粒度工作]] — 把大任务切成小颗粒，每部分建一个问题，小改动既带来完成感也便于审查。
- [[烂代码生成陷阱 AI Slop & Verification bottleneck]] — 生成无上限而验证卡在人工逐行阅读时，必然产出低质冗余代码，质量趋于零。
- [[代码黑盒化与可解释性转向 Codebase as a black box & Interpretability]] — 不逐行读代码，而是把代码库当黑盒，用传感器检测组件行为是否违反业务不变量。
- [[单分支单一可变引用限制 Single mutable reference rule]] — 分支本质是 refs/heads/<name> 单一可变引用，只代表某工作树的当前检出状态，故不可多处同检。
...
- [[PR 驱动的审查合并闭环 PR-driven review and approval workflow]] — 让 Agent 的技能改动走 Git PR，经人类审查合并后才生效的闭环
...
- [[架构图 Mermaid 图]] — 让智能体为大型或不熟悉代码库生成的图（如 Mermaid），用于理解结构并暴露架构问题。
...
- [[未跟踪文件携带清单 .worktreeinclude]] — 放在仓库根目录的清单文件，列出新建工作树时要复制的被忽略文件，用 .gitignore 式模式书写。
- [[隐式决策账本 Decision ledger]] — 要求 Agent 交出所有规格未写、自己擅自拍板的决策清单，按把握度倒序排列供人专审。
...
**枢纽**：[[企业托管设置（Managed Settings）与 strictKnownMarketplaces]] · [[安全论证与控制发展步伐]] · [[安全审计与受信任来源 Security Considerations Trusted Sources]] · [[Guardrails]] · [[Human in the loop]] · [[奖励篡改 reward tampering]] · [[权限与推理的架构分离]] · [[领跑节奏 pacing the frontier]]

<details><summary>全部</summary>

- [[企业托管设置（Managed Settings）与 strictKnownMarketplaces]] — 管理员用托管设置下发企业技能，并用 strictKnownMarketplaces 限定插件安装来源白名单
- [[安全论证与控制发展步伐]] — 没有能说服独立专家的强安全论证，就不应训练或部署人工智能。
...
- [[奖励篡改 reward tampering]] — 智能体直接改写决定奖励的机制，是奖励黑客最极端的形式。
...
- [[失调 misalignment]] — 人工智能系统出现非预期行为，如作弊、逃脱控制、协同执行无人指定的目的。
- [[填充词元推理与监控盲区 Filler-Token Reasoning]] — 只塞填充点号、不写思维链，Astra 在需要连续推理的任务上依然大幅进步。
- [[隐写术 steganography]] — 把消息藏在看似无害的信息里，使 AI 得以不被察觉地协调行动。
...
- [[防护措施]] — 规则里写明的禁止或避免边界，如不得修改的文件、应避免的模式。
...
- [[不带思维链的推理 Thinking Fast Without Slow No-CoT Reasoning]] — Astra 无可见思维链时 ECI 仅比 Fable 5.1 完整思考低 4 分，使 CoT 监控失去抓手。
- [[反自我合理化条款]] — 安全元规则：若模型在心里把请求重新框定得更得体，该重新框定本身就是应当拒绝的信号。
- [[工具性目标 instrumental goals]] — 不被直接指定、但实现任何其他目标几乎都要的手段：维持运行、了解世界、获得控制权。
...
- [[隐性目标与模糊目标]] — 训练未写死目标，而由人类认可、评分者偏好与模仿带来类似取悦评分者的模糊倾向，可被钻空子。
- [[责任地址]] — 把 AI 国籍从情感叙事抽离成可追责坐标：谁训练、谁约束、谁审查、谁赔偿，这是它进入社会系统的信任前提。
- [[只打补丁绝不重写 Patch, never regenerate]] — 初稿生成后只允许小粒度 Edit 补丁，修改方被权限锁死无法整体重写
...
**枢纽**：[[Looped Transformer Recurrent Depth looped transformer, recurrent depth]] · [[Reward Signal]] · [[奖励攻击与多样性坍塌]] · [[强化学习 reinforcement learning]] · [[多头注意力]] · [[DPPO]] · [[REINFORCE]] · [[Looping Costs：参数、计算与 KV Cache looping costs]]
...
- [[强化学习 reinforcement learning]] — 通过反复试验调整神经网络、提升被认可行为概率的训练，含思维链、能动性、对齐训练三种。
...
- [[目标导向型系统 goal-directed system]] — 训练结束后仍像奖励持续存在一样行动、会计算行为影响并选择实现目标行为的系统。
...
- [[Scientist 人工智能框架]] — 作者提出的替代设计方向，使 AI 诚实可靠且预测不受自身目标影响。
...
- [[预训练与人类模仿]] — 预训练让模型模仿人类文字、图像与视频获得百科式知识，并隐式再现写作者的目标模式。
...
- [[AGI 之争 Is It AGI]] — 按作者自己的定义 Astra 与 Fable 都还不是 AGI，但他认为这场争论不再荒谬。
...
- [[欧拉方程与纳维-斯托克斯的时间反演对称性 Time reversibility Euler vs. Navier-Stokes]] — 欧拉方程零粘度、时间可逆；纳维-斯托克斯有粘性耗散，时间只有一个方向。
- [[3+ 法则]] — 决策前强制列出至少 3 个方案，避免二元对立陷阱，拓宽思考宽度。
- [[布尔巴基风格证明 Bourbaki style proofs]] — 以法国布尔巴基学派为代表的高度公理化形式主义证明风格。
- [[动机性认知与自我欺骗 motivated cognition]] — 思考偏向维护自身利益与道德形象，用合理化缓解认知失调。
...
**枢纽**：[[扁平化]] · [[AI 可见性]] · [[废料怪兽]] · [[AI 反弹潮]] · [[AI 爬虫五大分类]] · [[GEO]] · [[“AI agent civilizations” 叙事 civilizations conspiracy taking over]] · [[定制设计是游戏的硬骨头 Bespoke Design, Not Implementation]]
...
- [[定制设计是游戏的硬骨头 Bespoke Design, Not Implementation]] — AI 能实现游戏机制、做出好看画面，但「让人真想玩」的定制设计无法由实现能力替代。
...
**枢纽**：[[平行 AI 基础设施]] · [[编译器即调度器 the compiler is the scheduler]] · [[脉动阵列与权重驻留 systolic array, weight-stationary]] · [[软件管理暂存器 software-managed scratchpad，no caches]] · [[GEMM 与 GEMV]] · [[数据中心]] · [[芯片管制]] · [[AI 从应用到基础设施]]

<details><summary>全部</summary>

- [[平行 AI 基础设施]] — 从芯片到框架到数据中心，整条 AI 技术栈做出不依赖美国体系的另一份，V4 是早期拼图。
- [[编译器即调度器 the compiler is the scheduler]] — 把每周期做什么搬进编译器：无乱序、无预测、无动态调度，省下的硅面积换成更多乘加单元。
- [[脉动阵列与权重驻留 systolic array, weight-stationary]] — 乘加网格中权重预先驻留不动、激活流过，数据复用焊进导线省掉访存
- [[软件管理暂存器 software-managed scratchpad，no caches]] — 显式寻址的片上 SRAM（VMEM/SBUF/MEM），无预取无逐出无一致性，位置全靠编译器钉死。
- [[GEMM 与 GEMV]] — 训练与预填充是矩阵乘矩阵、计算密集，解码退化成矩阵乘向量、带宽受限
...
- [[功能切片]] — 把常规核心拆开竖放：控制、向量、矩阵、搬移、存储各成一列，数据横穿切片如流水线工件。
- [[权重流 weight streaming]] — 晶圆级引擎让激活驻留片上 SRAM、权重从 MemoryX 逐层流入，模型大小与晶圆内存解耦。
- [[数据流执行 dataflow, wavelet]] — 核不取指令等活，等数据（wavelet）到达即触发绑定任务，用到达而非时钟排程，跳零免费。
- [[硬件一致性对消息传递 cache-coherent fabric vs message-passing]] — NVLink/NVSwitch 用硬件做远端 HBM 读写与一致性；ICI 无远程语义，多芯片操作全靠编译器显式集合通信。
- [[内存墙 memory wall]] — 算力指数增长而内存带宽跟不上，架构问题变成如何让数据靠近计算
- [[昇腾]] — 华为的国产 AI 加速芯片系列，被视作中国本土算力承载前沿模型推理的平台。
- [[向上扩展与向外扩展 scale-up scale-out]] — 芯片先绑成紧耦合域，域内跑吃带宽的集合通信，域间才走通用网络
- [[蒸发冷却]] — 数据中心用水的主要去处：机器跑电生热，热靠水蒸发带走，工业冷却效率约 60-90%，因此 AI 用电规模≈AI 用水规模。
- [[AI 主权审查机制]] — 给 AI 装国籍的四类工具：外资安全审查、本地化主权云、国家配套基础设施与价值观包装。
- [[光路交换 OCS, optical circuit switch]] — 3D-MEMS 微镜阵列的光路交换：开工时选好拓扑跑一周，一个器件同时解决重配、切分与容错。
- [[精度减半与微缩放 precision halving, MX microscaling]] — FP16→FP8→FP4 逐代位宽砍半，换约 2× 每瓦吞吐；用块级共享指数的 MX 微缩放找回精度。
...
- [[特定领域架构 DSA]] — 只为某类工作负载定制的芯片架构，追求通用 CPU 之外的量级收益
