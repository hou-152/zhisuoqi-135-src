...
- 标题：都承诺提供6GW的运维能力
- 来源：Jacob Peake
- 原文：https://www.jacobpeake.com/ai-chip-architectures
- 作者：Jacob Peake
- 类型：文章
- 摘要：NVIDIA and Google use different methods to connect chips, with NVIDIA focusing on hardware coherence and Google on message-passing. AMD and Cerebras take unique approaches to memory and computation, enabling large models to run efficiently on fewer devices. This leads to simpler programming and scaling strategies compared to traditional GPU training setups.
- 收藏于：2026/9/15 03:24:55
- 抓取：Reader 快照（2026/9/15）
- 字数：137800

---
在 2018 年[国际计算机体系结构研讨会](https://iscaconf.org/)上，***[John Hennessy](https://en.wikipedia.org/wiki/John_L._Hennessy)***和***[David Patterson](https://en.wikipedia.org/wiki/David_Patterson_(computer_scientist))***发表了他们的[图灵](https://amturing.acm.org/)演讲：***[“计算机体系结构的新黄金时代”](https://dl.acm.org/doi/10.1145/3282307)***。


上世纪80年代，***亨尼西***和***帕特森***进行图灵奖获奖研究时，  

单线程CPU性能每年增长52%。到2018年，随着***[摩尔定律](https://en.wikipedia.org/wiki/Moore%27s_law)***和***[丹纳德缩放定律](https://en.wikipedia.org/wiki/Dennard_scaling)***的终结，这一增长率降至3%。


当时需要***特定领域架构***（DSA）。他们举的例子是谷歌的***[TPU v1](https://en.wikipedia.org/wiki/Tensor_Processing_Unit)***，该架构已投入生产：在神经网络推理方面，其吞吐量是CPU的29倍，能效提高了80倍。最后他们预测：***“未来十年将迎来计算机架构的寒武纪式爆发。”***


这一预测已成为现实。如今，数十种架构正在积极研发中，包括***GPU***、***TPU***、***LPU***、***NPU***、***DPU***、***ASIC***、***晶圆级引擎***、***可重构数据流***、***神经形态***计算、***光子计算***和***模拟计算***。这些架构尤其专注于***人工智能***的计算。


目前已获得实际部署的架构有：***GPU***（NVIDIA、AMD）、***脉动阵列加速器***（TPU、Trainium）、***Cerebras 晶圆级引擎***和***Groq LPU***。


***英伟达***显然遥遥领先； ***AMD***[紧随其后， OpenAI](https://openai.com/index/openai-amd-strategic-partnership/)和[Meta](https://www.amd.com/en/newsroom/press-releases/2026-2-24-amd-and-meta-announce-expanded-strategic-partnersh.html)都承诺提供6GW的运维能力。TPU***用于***训练Gemini，并将[为Anthropic提供多达一百万个芯片；](https://www.anthropic.com/news/expanding-our-use-of-google-cloud-tpus-and-services) ***Anthropic***还在[超过一百万个***Trainium***芯片](https://techcrunch.com/2026/03/22/an-exclusive-tour-of-amazons-trainium-lab-the-chip-thats-won-over-anthropic-openai-even-apple/)上运行Claude。Cerebras目前[为OpenAI提供推理服务](https://openai.com/index/cerebras-partnership/)； ***Groq LPU***已[通过一项价值200亿美元的收购并入英伟达](https://www.datacenterdynamics.com/en/news/nvidia-builds-out-lpu-chip-team-following-20bn-groq-acquihire-announcement-rumored-for-gtc/)。


本文旨在调查这些不同的方法——它们的*理念*、*架构*、*扩展方法（向上扩展和向外扩展）*以及*软件堆栈（如何对芯片进行编程）*。


人工智能计算主要依赖于***矩阵乘法***。Transformer 模型是一系列矩阵乘法操作：***Q/K/V 投影***、***注意力机制***、输出投影、***前馈神经网络******（FFN）***，并穿插着元素级操作：归一化、激活函数、残差加法。训练一个前沿模型需要执行一系列操作。102510^{25}1 025乘加运算（matmuls 是一系列乘加运算）。


*这些矩阵乘法运算的形状*取决于工作负载。***训练阶段***会将一批序列逐层向前推送，反向传播损失，并更新权重，数千个词元同时流经同一个权重矩阵。***预填充***是推理的快速输入阶段：在生成第一个输出词元之前，将完整的输入序列一次性投影到模型中。训练和预填充阶段都会将大量词元堆叠到同一个权重矩阵上，因此每一层的运算都是大规模的***矩阵***乘法（GEMM），具有很高的算术强度（计算密集型）。***解码***是自回归的：模型一次输出一个词元，每个词元都以其之前的每个词元为条件，并且只有在生成了第*N*个词元之后才能开始生成第*N+1*个词元。每一步只投影一个词元，因此每个矩阵乘法运算都变成了***矩阵向量***乘积（GEMV）。生成一个词元需要完整遍历模型中的每个权重，以及完整读取键值缓存以获取注意力信息。与预填充相比，算术运算强度下降了几个数量级。


推理系统通过批量处理词元来恢复部分运算强度，从而将GEMV提升回GEMM：***连续批量处理***堆叠多个用户的解码步骤，***推测性解码***每次请求堆叠K个草拟词元并在一次遍历中验证它们，而***多词元预测***则将同样的技巧集成到模型内部。这提高了矩阵乘法单元的利用率，并提升了Ops/B。对于连续批量处理，每个用户的请求仍然读取其自身的KV缓存，因此长上下文解码的带宽限制从权重带宽限制转变为KV带宽限制。


这里架构上的问题在于***如何将计算任务转移***到矩阵乘法运算速度足够快的位置。这就是所谓的***内存墙***：计算能力呈指数级增长，而内存带宽却没有相应提升。
...
The NVIDIA GPU is a ***massively parallel processor***. The philosophy is that a programmable chip with thousands of threads, orchestrated by a host CPU and exposed through ***[CUDA](https://docs.nvidia.com/cuda/cuda-c-programming-guide/)***, is the right machine to run ***parallelisable*** workloads. Each generation adds acceleration primitives onto programmable ***Streaming Multiprocessors*** without changing the programming model. The same chip trains transformers, serves inference, renders graphics, and runs scientific simulation (***accelerated computing***).
...
![Blackwell B200 单芯片布局——GigaThread Engine 贯穿芯片中心，将芯片分为左右两半；每一半都有各自的 L2 缓存带，其上下两侧分别是 GPC 集群；HBM3e 内存堆栈沿外缘排列，并集成内存控制器。NVLink 和一个小型 PCIe Gen 6 主机链路位于顶部；底部的 NV-HBI 桥接器与镜像的第二个芯片相连，从而完成封装。](https://www.jacobpeake.com/diagrams/nvidia-gpu-die.png)Blackwell B200 单芯片布局——GigaThread Engine 贯穿芯片中心，将芯片分为左右两半；每一半都有各自的 L2 缓存带，其上下两侧分别是 GPC 集群；HBM3e 内存堆栈沿外缘排列，并集成内存控制器。NVLink 和一个小型 PCIe Gen 6 主机链路位于顶部；底部的 NV-HBI 桥接器与镜像的第二个芯片相连，从而完成封装。
![放大查看一个流式多处理器——四个子分区，每个子分区都有自己的 warp 调度器、分发、寄存器文件和张量内存，利用共享的 L1/SMEM 和下面的 TMA。](https://www.jacobpeake.com/diagrams/nvidia-sm.png)放大查看一个流式多处理器——四个子分区，每个子分区都有自己的 warp 调度器、分发、寄存器文件和张量内存，利用共享的 L1/SMEM 和下面的 TMA。
...
These cores execute ***fused matrix multiply-accumulate*** on small matrix tiles, D=A⋅B+CD = A \cdot B + CD=A⋅B+C The full matmul is broken into output tiles: to produce one output tile, a kernel walks the shared inner dimension KKK, drawing AAA from a row-strip of the left input matrix and BBB from a column-strip of the right, and folds each partial product into a running accumulator. CCC holds the partial sum so far, DDD is the updated value carried into the next step. After the inner loop completes, DDD is one finished tile of the full output matrix; the whole matmul is built from many of these tile MMAs.


Tile shapes are written **M × N × K**, M×NM \times NM×N is the output tile size, and KKK is how much of the inner dimension the instruction contracts over in one fire; the rest of the matmul's KKK axis is walked by the kernel's inner loop. The accumulator is sticky across that loop: each MMA's output DDD becomes the next MMA's input CCC, so the equation is really C←A⋅B+CC \leftarrow A \cdot B + CC←A⋅B+C in place: successive instructions fold their partial products into the same storage until the K-axis is fully walked.
...
* ***Bet 1: Programmability.*** The workload is a moving target (attention variants, novel model architectures), so keep every block programmable and let the developer write [CUDA](https://docs.nvidia.com/cuda/cuda-c-programming-guide/). Even the specialised units are exposed *through* that model rather than as fixed-function blocks.
...
![NVL72——72 个 Blackwell GPU 位于一排 NVSwitch ASIC 下方，这些 ASIC 构成一个无阻塞交叉开关，因此任何 GPU 都可以以 NVLink 的全带宽寻址任何其他 GPU 的 HBM。整个架构运行在无源铜背板上：约 5184 根线缆采用盲插方式连接，总带宽约为 130 TB/s，与同等光纤方案相比，可节省约 20 kW 的收发器功耗。](https://www.jacobpeake.com/diagrams/nvidia-scale-up.png)NVL72——72 个 Blackwell GPU 位于一排 NVSwitch ASIC 下方，这些 ASIC 构成一个无阻塞交叉开关，因此任何 GPU 都可以以 NVLink 的全带宽寻址任何其他 GPU 的 HBM。整个架构运行在无源铜背板上：约 5184 根线缆采用盲插方式连接，总带宽约为 130 TB/s，与同等光纤方案相比，可节省约 20 kW 的收发器功耗。
...
![DGX SuperPOD——八个 NVL72 机架（共 576 个 GPU）位于 Quantum-X800 InfiniBand 主干网卡下方。每个 GPU 的横向扩展采用 800 Gbps 的 ConnectX-8 网卡；机架间链路通过 OSFP-RHS 可插拔光收发器传输，延迟仅为微秒级，远低于上方机架内 NVLink 架构的纳秒级延迟。](https://www.jacobpeake.com/diagrams/nvidia-scale-out.png)DGX SuperPOD——八个 NVL72 机架（共 576 个 GPU）位于 Quantum-X800 InfiniBand 主干网卡下方。每个 GPU 的横向扩展采用 800 Gbps 的 ConnectX-8 网卡；机架间链路通过 OSFP-RHS 可插拔光收发器传输，延迟仅为微秒级，远低于上方机架内 NVLink 架构的纳秒级延迟。
...
***[CUDA](https://docs.nvidia.com/cuda/cuda-c-programming-guide/)*** is the natural programming model for a ***massively parallel*** processor. You write a kernel (one piece of code executed once per thread) and launch it across thousands of threads organised into blocks and warps; the programmer decides what they share, when they synchronise, and which piece of the problem each one handles. That is why the abstraction has barely changed in eighteen years, and why every CUDA kernel written since 2007 would still compile and run on Blackwell.


That continuity is both the moat and the constraint. Each new generation introduces new hardware (Tensor Cores, TMA, TMEM) onto the same kernel-and-warps model, exposed as intrinsics in ***[PTX](https://docs.nvidia.com/cuda/parallel-thread-execution/)*** and ***[SASS](https://docs.nvidia.com/cuda/cuda-binary-utilities/)***: `mma.sync`, `wgmma.mma_async`, and so on. NVIDIA cannot radically rethink the SM because too much code depends on it; in return, every investment in CUDA software compounds across generations.


On top of PTX sits a stack constructed over two decades. ***[cuBLAS](https://docs.nvidia.com/cuda/cublas/)*** and ***[cuDNN](https://developer.nvidia.com/cudnn)*** for math and DNN primitives; ***[CUTLASS](https://github.com/NVIDIA/cutlass)***, encoding decades of GEMM expertise in templated C++; ***[TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM)*** for paged attention, in-flight batching, and speculative decoding; framework bindings through ***[PyTorch](https://pytorch.org/)***, ***[Triton](https://triton-lang.org/)***, and ***[JAX](https://github.com/jax-ml/jax)***.


***[FlashAttention](https://arxiv.org/abs/2205.14135)***, one of the most important algorithmic rewrites in modern AI, tiles attention to avoid materialising the O(N2)O(N^2)O(N2) matrix. Its four generations (FA1 through FA4) have each been hand-optimised for the latest NVIDIA silicon (FA3 for Hopper's async pipelines, FA4 for Blackwell), with ports to other hardware trailing by months or years.
...
The ***[TPU](https://en.wikipedia.org/wiki/Tensor_Processing_Unit)*** is a ***matrix multiplication machine***. The philosophy is, rather than a programmable chip that can run any massively-parallel workload, focus on a single primitive (dense matrix-multiplication on a large [systolic array](https://en.wikipedia.org/wiki/Systolic_array)) and let the ***[XLA](https://openxla.org/xla)*** compiler plan every cycle and every byte of memory ahead of time. No hardware scheduler, no cache, no threads/warps. Each generation grows the pod, with thousands of chips wired through the ***ICI*** interconnect into one coherent machine. A TPU has no ambition to render graphics or run scientific simulation; it exists to train and serve Google's workloads (search, translation, recommendation, Gemini) more efficiently per watt than any general-purpose alternative.
...
Google's ***Multislice*** framework, plumbed into ***[XLA](https://openxla.org/xla)***, lets a single SPMD program span multiple slices in different pods; the compiler emits hierarchical collectives (ring all-reduce inside each slice, higher-level reduce across). The structure is exactly the trick for hiding the ICI/DCN bandwidth gap: as much work as possible stays inside the slice over fast ICI, leaving only the cross-slice residual to pay the slow-fabric cost.


Above this sits ***[Pathways](https://arxiv.org/abs/2203.12533)***. Where NCCL + Slurm + Megatron-style schedulers drive SPMD from many controllers, [Pathways](https://arxiv.org/abs/2203.12533) drives the entire job from ***one*** client and virtualises multiple "islands" (pods with their own ICI domains) connected over DCN. It does gang scheduling, elastic training (when a slice fails, OCS reshapes the topology and [Pathways](https://arxiv.org/abs/2203.12533) resumes from the last checkpoint on the new shape), and cross-region orchestration. ***Gemini Ultra*** was the first frontier model trained across multiple datacenters; [Pathways](https://arxiv.org/abs/2203.12533) stitches them into one synchronous SPMD job.
...
The TPU stack is ***compiler-driven*** where CUDA is ***kernel-driven***. On a GPU, the developer writes the kernel and the framework strings kernels together; the compiler's job is mostly local. On a TPU, the developer writes a numerical program in ***[JAX](https://github.com/jax-ml/jax)*** and ***[XLA](https://openxla.org/xla)*** is responsible for everything below it: which operations fuse, where each tensor lives, how it is laid out across the 2D vector registers, when DMAs from HBM to VMEM issue, how the 322-bit VLIW bundles are scheduled, how the program shards across thousands of chips. There is no hardware fallback: no warp scheduler, no cache, no out-of-order engine to paper over a bad schedule. The compiler is the system. The trade-off is the central one of the architecture: ***XLA gets closer to the theoretical ceiling without hand-tuning, but closing the remaining gap is harder***.


The compilation path is ***[JAX](https://github.com/jax-ml/jax) → JAXpr → [StableHLO](https://github.com/openxla/stablehlo) → HLO → LLO → VLIW bundles***. ***[JAX](https://github.com/jax-ml/jax)*** traces a Python function into a typed functional IR (JAXpr) under `jit`, lowers it to ***[StableHLO](https://github.com/openxla/stablehlo)*** (the OpenXLA-standardised, versioned op-set of ~100 statically-shaped primitives that all front-ends now emit), which XLA ingests as ***HLO*** and runs through its pass pipeline: ***operation fusion*** (collapse pointwise + reduction + matmul into one kernel so intermediates never hit HBM), ***layout assignment*** (decide the 2D tiling of every tensor so it streams into the MXU without a transpose: substantially harder than on 1D SIMD machines because both the registers and the systolic inputs are 2D), ***buffer assignment*** (every tensor pinned to either VMEM, CMEM, or HBM with overlap windows pre-computed), ***SPMD partitioning***, and finally a VLIW scheduler that fills all eight slots of every bundle. HLO lowers to ***LLO*** (Low-Level Optimizer), the TPU-specific IR, and LLO emits the final VLIW stream. A well-compiled program overlaps MXU systolic execution, VPU element-wise math, and HBM↔VMEM DMA in the same bundle every cycle.


Multi-chip execution is ***SPMD***: one program, sharded data, hierarchical collectives, emitted by ***[GSPMD](https://arxiv.org/abs/2105.04663)*** (now being replaced by ***[Shardy](https://github.com/openxla/shardy)***, an MLIR-native successor that lands as the default in early 2026). The user expresses sharding declaratively with ***Mesh*** + ***PartitionSpec*** annotations on a few key tensors; the compiler propagates shardings through the rest of the graph and inserts all-reduces, all-gathers, and reduce-scatters where the layout changes. When the compiler picks a wrong collective, ***shard\_map*** drops the user into ***manual SPMD*** (per-device code with explicit local shapes and explicit collectives), composable inside `jit` so a single kernel can be hand-partitioned without giving up auto-partitioning everywhere else. This is the inverse of the PyTorch idiom: ***[FSDP](https://pytorch.org/docs/stable/fsdp.html)*** and ***[DeepSpeed](https://github.com/microsoft/DeepSpeed)*** wrap the model in a runtime that issues collectives at module boundaries; GSPMD/Shardy partitions the whole graph as a compiler problem.


***[Pallas](https://docs.jax.dev/en/latest/pallas/)*** is the escape hatch: JAX's kernel-writing language, broadly the TPU equivalent of ***[Triton](https://triton-lang.org/)*** on GPUs. Pallas kernels are written in JAX-flavoured Python, lowered through ***[Mosaic](https://docs.jax.dev/en/latest/pallas/)*** (the MLIR-based TPU backend) to LLO, and embedded back into HLO as a custom op. It exists because XLA cannot always synthesise the optimum for novel attention variants, fused MoE dispatch, or anything that demands manual VMEM tiling and DMA scheduling: a ***FlashAttention-class*** optimisation, where the win is in the schedule and not the algebra. ***Pallas:Mosaic-GPU*** targets H100/Blackwell with the same front-end, so a kernel author can write once and lower to either substrate. The library tier above this is uniformly JAX-native: ***[Flax NNX](https://github.com/google/flax)*** for modules, ***[Optax](https://github.com/google-deepmind/optax)*** for optimisers, ***[Orbax](https://github.com/google/orbax)*** for asynchronous distributed checkpointing, ***[Grain](https://github.com/google/grain)*** for input pipelines, ***[Tunix](https://github.com/google/tunix)*** for post-training/RL, ***[Qwix](https://github.com/google/qwix)*** for quantisation. Google's reference training stacks (***[MaxText](https://github.com/AI-Hypercomputer/maxtext)*** for LLMs including DeepSeek-V3-class MoE, and ***[MaxDiffusion](https://github.com/AI-Hypercomputer/maxdiffusion)*** for Flux, Wan 2.1) sit at the top, in pure JAX; ***[Pathways](https://arxiv.org/abs/2203.12533)*** sits beneath, exposed to the user as ***[pathwaysutils](https://pypi.org/project/pathwaysutils/)***, so a single Python client can drive a job across thousands of chips and several pod-islands without giving up the JAX programming model.


The PyTorch path is real but second-class. ***[torch\_xla](https://github.com/pytorch/xla)*** uses a ***LazyTensor*** mechanism: every PyTorch op records into an HLO graph that compiles on the next barrier, with the compiled artifact cached by graph-shape hash. PyTorch/XLA 2.x added ***GSPMD-style sharding annotations***, ***`torch.compile` integration*** through an XLA backend, a ***JAX bridge***, and (PyTorch/XLA 2.7) C++11-ABI builds with materially faster tracing. The gap to JAX is real (JAX's primitives map more cleanly to [StableHLO](https://github.com/openxla/stablehlo), and complex parallelism strategies are better-covered), which is why ***[vLLM TPU](https://blog.vllm.ai/2025/10/16/vllm-tpu.html)*** (powered by the ***[tpu-inference](https://github.com/vllm-project/tpu-inference)*** plugin announced at Cloud Next 2025) lowers ***every*** model, JAX-defined or PyTorch-defined, through a ***unified JAX→XLA path***. ***[TorchTPU](https://developers.googleblog.com/torchtpu-running-pytorch-natively-on-tpus-at-google-scale/)***, announced April 2026, is Google's response: a native PyTorch experience with eager mode, `torch.distributed`, and `torch.compile` over XLA, on track to replace torch\_xla.
...
The ***[AMD Instinct](https://www.amd.com/en/products/accelerators/instinct.html) GPUs*** are built on a different bet from NVIDIA: where NVIDIA each generation expands what each SM can *do*, AMD has held the ***Compute Unit*** conservative since GCN (2012) and reinvested into the package: matched or beat the contemporary NVIDIA flagship on HBM capacity every generation since 2021; the first ***3D-stacked*** datacenter GPU (CDNA 3); the first coherent ***CPU+GPU APU*** (MI300A); and an ***open ecosystem*** (ROCm, HIP, OCP MX, UALink).
...
![AMD Instinct MI355X (CDNA 4) package floorplan — eight XCDs (Accelerator Complex Dies, ~32 active CUs each) hybrid-bonded via TSMC SoIC onto two IOD base dies. The IODs carry the 256 MB Infinity Cache (128 MB per IOD), HBM PHYs, Infinity Fabric, and PCIe Gen 5. HBM3E stacks line the perimeter; 8 stacks of 12-Hi for 288 GB total.](https://www.jacobpeake.com/diagrams/amd-gpu-chip.png)AMD Instinct MI355X (CDNA 4) package floorplan — eight XCDs (Accelerator Complex Dies, ~32 active CUs each) hybrid-bonded via TSMC SoIC onto two IOD base dies. The IODs carry the 256 MB Infinity Cache (128 MB per IOD), HBM PHYs, Infinity Fabric, and PCIe Gen 5. HBM3E stacks line the perimeter; 8 stacks of 12-Hi for 288 GB total.
![One Compute Unit in zoom — a single Scheduler dispatches wave64 wavefronts across four SIMD16 vector engines over four cycles; each SIMD has its own Matrix Core (MFMA) running matmul beside it. A shared Scalar Unit, three register files (VGPR / SGPR / AGPR), a 160 KB LDS scratchpad, and a 32 KB L1 vector cache complete the recipe — the same shape AMD has held since GCN in 2012.](https://www.jacobpeake.com/diagrams/amd-cu.png)One Compute Unit in zoom — a single Scheduler dispatches wave64 wavefronts across four SIMD16 vector engines over four cycles; each SIMD has its own Matrix Core (MFMA) running matmul beside it. A shared Scalar Unit, three register files (VGPR / SGPR / AGPR), a 160 KB LDS scratchpad, and a 32 KB L1 vector cache complete the recipe — the same shape AMD has held since GCN in 2012.
...
![AMD Helios — 72 MI455X GPUs sit beneath a row of UALink switches in an Open Rack Wide chassis, joined into one coherent UALink memory domain. At launch the fabric runs over UALoE (Infinity Fabric tunnelled over Ethernet) as a stopgap until native UALink switching silicon ships in 2027. Each GPU carries a Pensando Vulcano 800 NIC on the way out.](https://www.jacobpeake.com/diagrams/amd-scale-up.png)AMD Helios — 72 MI455X GPUs sit beneath a row of UALink switches in an Open Rack Wide chassis, joined into one coherent UALink memory domain. At launch the fabric runs over UALoE (Infinity Fabric tunnelled over Ethernet) as a stopgap until native UALink switching silicon ships in 2027. Each GPU carries a Pensando Vulcano 800 NIC on the way out.
...
![AMD scale-out — Helios racks talk to each other over standard Ethernet anchored on the open Ultra Ethernet (UEC) standard. UET replaces RoCEv2 with a clean redesign of RDMA semantics. Each GPU carries a Pensando Vulcano 800 NIC (PCIe Gen 6, 800 GbE, UEC 1.0); the inter-rack switching is Broadcom Tomahawk 6 with co-packaged optics. AMD owns the NIC tier, switches and optics are partner silicon.](https://www.jacobpeake.com/diagrams/amd-scale-out.png)AMD scale-out — Helios racks talk to each other over standard Ethernet anchored on the open Ultra Ethernet (UEC) standard. UET replaces RoCEv2 with a clean redesign of RDMA semantics. Each GPU carries a Pensando Vulcano 800 NIC (PCIe Gen 6, 800 GbE, UEC 1.0); the inter-rack switching is Broadcom Tomahawk 6 with co-packaged optics. AMD owns the NIC tier, switches and optics are partner silicon.
...
Above HIP sits a library tier structured to mirror NVIDIA's, one-to-one by name: ***[rocBLAS](https://github.com/ROCm/rocBLAS)*** for cuBLAS; ***[hipBLASLt](https://github.com/ROCm/hipBLASLt)*** for cuBLASLt; ***[MIOpen](https://github.com/ROCm/MIOpen)*** for cuDNN; ***[RCCL](https://github.com/ROCm/rccl)*** for NCCL; ***Composable Kernel*** (and its modern ck-tile DSL) for CUTLASS; rocprofv3 / rocprof-sys / rocprof-compute for the Nsight family. There is no first-party analog of TensorRT-LLM, though. AMD's answer is to back ***[vLLM](https://github.com/vllm-project/vllm)*** as the open-source serving engine and ship AMD-specific operators (***AITER***) that plug into it; the dedicated ROCm CI for vLLM took test-pass rate from 37% to 93% across early 2026.
...
***FlashAttention*** is the load-bearing case. ***FA2*** is production on MI300X via Composable Kernel; PyTorch defaults to CK or AOTriton on ROCm. ***FA3*** (Hopper-tuned) is partially supported via AITER + CK, but Dao-AILab's canonical implementation remains CUDA-only. ***FA4*** (Blackwell, March 2026) has no ROCm port at all. ***[HipKittens](https://hazyresearch.stanford.edu/blog/2025-11-09-hk)***, Hazy Research's MI355X port of ThunderKittens (November 2025), claims forward-pass parity with hand-tuned AITER in ~500 lines. The pattern: open-source academic kernels close the AMD tail months after NVIDIA's, not years.
...
The market has priced this honestly. Mistral's Le Chat (~1,100 tokens/s), Perplexity Sonar, and Meta's Llama API all pay for the latency; in January 2026 OpenAI signed for ***750 MW of CS-3 capacity through 2028***, [reported above $10B](https://www.cnbc.com/2026/01/14/cerebras-scores-openai-deal-worth-over-10-billion.html) at signing and [since grown past $20B](https://finance.yahoo.com/technology/ai/articles/cerebras-systems-openai-tout-20b-040208708.html), the largest endorsement wafer-scale has ever received. The first flagship to ship on that capacity is ***[GPT-5.6 Sol](https://openai.com/index/gpt-5-6/)***, launched July 2026 at a quoted 750 tokens/s.
...
Where does that leave the wafer? Owning a real niche, honestly won: ***batch-one decode speed***, independently verified, paid for by customers who price latency above cost.


### AWS Trainium


***Annapurna Labs***, the team behind AWS's ***Nitro*** cards and ***Graviton*** CPUs, built ***Trainium*** as a ***fast-follower***. The compute core takes the TPU's proven playbook (a 128×128 weight-stationary systolic array, software-managed scratchpads, whole-program compilation) down to sharing Google's ***[XLA](https://openxla.org/xla)*** compiler outright. The scale-out fabric is the ***Nitro***-offloaded network that already carries the rest of AWS. What is genuinely Amazon's is narrow and deliberate: dedicated collective-communication silicon bolted onto the borrowed core, and the vertical integration to price a chip that only has to beat NVIDIA *inside AWS*.
...
![AWS Trainium2 package floorplan — two compute dies sit side by side on a CoWoS interposer, four NeuronCore-v3 per die (eight per chip); each die is flanked by two HBM3 stacks via memory controllers on the outer edges. A central NeuronLink block carries the on-package die-to-die link and the chip-to-chip torus ports; a small PCIe / Nitro EFA strip on top is the path to the host and the scale-out fabric.](https://www.jacobpeake.com/diagrams/aws-trainium-chip.png)AWS Trainium2 package floorplan — two compute dies sit side by side on a CoWoS interposer, four NeuronCore-v3 per die (eight per chip); each die is flanked by two HBM3 stacks via memory controllers on the outer edges. A central NeuronLink block carries the on-package die-to-die link and the chip-to-chip torus ports; a small PCIe / Nitro EFA strip on top is the path to the host and the scale-out fabric.
![One NeuronCore-v3 in zoom — the 128×128 weight-stationary Tensor Engine sits at the centre, fed operands from the SBUF State Buffer (128 partitions) and draining partial sums into the small PSUM accumulator. The Vector, Scalar, and programmable GPSIMD engines run alongside it off the same SBUF; 128 DMA engines and a Sync Engine stage tiles from HBM, and a bank of CC-Cores drives the NeuronLink ports for collectives concurrently with compute.](https://www.jacobpeake.com/diagrams/aws-trainium-neuroncore.png)One NeuronCore-v3 in zoom — the 128×128 weight-stationary Tensor Engine sits at the centre, fed operands from the SBUF State Buffer (128 partitions) and draining partial sums into the small PSUM accumulator. The Vector, Scalar, and programmable GPSIMD engines run alongside it off the same SBUF; 128 DMA engines and a Sync Engine stage tiles from HBM, and a bank of CC-Cores drives the NeuronLink ports for collectives concurrently with compute.
The ***Tensor Engine*** owns the matmul FLOPs; the other three engines own everything else. It is a 128×128 grid of processing elements (16,384 MACs) run weight-stationary: one operand tile is loaded into the array and held in place (`LoadStationary`), the other streams through it (`MultiplyMoving`), and partial sums land in ***PSUM***, a small accumulator SRAM the engine can read-add-write so a contraction longer than 128 folds into place along the KKK axis. This is the same D=A⋅B+CD = A \cdot B + CD=A⋅B+C tile MMA at the heart of every matmul accelerator; but where NVIDIA wraps it in the warp hierarchy and Google issues it from a VLIW bundle, Trainium exposes it as a pair of explicit instructions against a named scratchpad.
...
![AWS Trainium scale-out — UltraServers connect through Elastic Fabric Adapter NICs offloaded to the Nitro cards, over standard Ethernet rather than InfiniBand. The SRD transport sprays each flow across up to 64 paths and delivers reliably but out-of-order, sidestepping head-of-line blocking. The 10p10u UltraCluster fabric (~10 petabits/s at under 10 microseconds) ties hundreds of thousands of chips together; Project Rainier is ~500,000 Trainium2 chips across multiple US datacenters for Anthropic.](https://www.jacobpeake.com/diagrams/aws-trainium-scale-out.png)AWS Trainium scale-out — UltraServers connect through Elastic Fabric Adapter NICs offloaded to the Nitro cards, over standard Ethernet rather than InfiniBand. The SRD transport sprays each flow across up to 64 paths and delivers reliably but out-of-order, sidestepping head-of-line blocking. The 10p10u UltraCluster fabric (~10 petabits/s at under 10 microseconds) ties hundreds of thousands of chips together; Project Rainier is ~500,000 Trainium2 chips across multiple US datacenters for Anthropic.
...
Trainium's software makes the borrowing explicit: the ***[Neuron SDK](https://awsdocs-neuron.readthedocs-hosted.com/)*** is a ***compiler-first stack built on the same OpenXLA foundation as the TPU***. The Neuron compiler (`neuronx-cc`) ingests XLA HLO graphs and lowers them to a ***NEFF*** binary that the Neuron runtime loads onto the NeuronCores; the front-end IR is Google's, and Google's own OpenXLA announcements list Trainium as a first-class PJRT device alongside the TPU. ***torch-neuronx*** runs PyTorch through [PyTorch/XLA](https://github.com/pytorch/xla)'s LazyTensor tracing (record ops, compile the graph at a step boundary), and ***jax-neuronx*** lowers JAX through [StableHLO](https://github.com/openxla/stablehlo). On the spectrum from kernel-driven CUDA at one pole to whole-program XLA at the other, Trainium sits almost on top of the TPU: the compiler is the system, and it is largely the same compiler.


Where it diverges is the escape hatch. XLA alone cannot always synthesise the optimum for a novel attention variant or a fused MoE dispatch, so Neuron ships ***NKI (Neuron Kernel Interface)***, a Python, tile-level kernel language that exposes the four engines and the SBUF/PSUM scratchpads directly. It is Trainium's ***[Pallas](https://docs.jax.dev/en/latest/pallas/)*** (or its ***[Triton](https://triton-lang.org/)***): the same idea of a tile DSL that drops beneath the whole-program compiler when a kernel's win is in the schedule, not the algebra. Below it, a ***collective-communication library*** maps all-reduce and all-to-all onto the CC-Cores and the NeuronLink topology (the NCCL analogue), and ***NeuronX Distributed*** provides the sharded-training layer.
...
That pivot is the tell about what the architecture is for. The LPU is ***inference-only*** by construction (Ross's framing is that training is a local game and inference a global one), and it is unbeaten at a single thing: single-user decode latency. Independent measurement backs the claim, with ***[Artificial Analysis](https://artificialanalysis.ai/providers/groq)*** clocking Groq among the fastest token-per-second providers on open models. It is badly matched to the rest: a model that will not fit in a rack of SRAM, a workload that wants big batches for throughput-per-dollar, or dynamic control flow a static schedule cannot express. MoE is served, but its data-dependent expert routing sits awkwardly against a compiler that wants to know everything in advance, and Groq has published little on how it reconciles the two.
...
* **Cerebras 打破了表格的坐标轴。**它完全没有采用 HBM：44 GB 的片上 SRAM，总读写速度为 21 PB/s，每瓦密集浮点运算约 1.3 字节，而 GPU 的密集浮点运算性能则接近 0.002。成本在同一行中也显而易见：总内存容量甚至低于单个 H200，每瓦密集浮点运算性能落后于所有同类 GPU，并且由于其相干域仅限于晶圆本身，因此扩展性方面没有一列数据。
* **Trainium 的竞争优势在于经济性，而非规格参数。**虽然单芯片性能略逊一筹（Trn2 的 1.3 PF FP8 性能约为 MI355X 的四分之一），但 Trn2 UltraServer 已于 2024 年与 NVL72 一起实现了 64 芯片机架级扩展，它采用的是消息传递环面而非相干交叉开关结构；而 Trn3 则转向了交换式 NeuronSwitch 架构。AWS 拥有从 Nitro 卡到 API 的每一层，并且其核心租户（Anthropic，拥有超过一百万个 Trainium2 芯片）已在尖端规模上验证了这一点。
* **Groq 通过牺牲内存容量来换取 SRAM 带宽，然后根据芯片数量扩展内存池。**第一代 GroqRack 仅提供 14 GB 内存，分布在 64 个激活芯片上；Groq 3 LPX 则将其扩展到 128 GB，分布在 256 个芯片上，总 SRAM 带宽为 40 PB/s。其 12 TB DDR5 内存层以及与 Rubin 的搭配表明，LPU 是大型内存 GPU 机架的补充，而非替代。
