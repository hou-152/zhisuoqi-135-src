...
- 数学类：Mystery Game Puzzles 84%（第二名 59%）、Epoch 的 EBR-Bench 第二次跑就 100%（人类最好要五次）、Induction 逼近饱和 88%（Sol 43%、Fable 5.1 33%）。
  - 原话：「and got 84% on Mystery Game Puzzles where the second highest score is 59%.」
  - 原话：「In Epoch’s EBR-Bench, Astra got 100% on its second run, outpacing the best human who required five attempts.」
  - 原话：「versus previous high of 43% for Sol, with Fable 5.1 at 33%.」
- 语言与视觉类：Extended NYT Connections 98.1（Fable 90）、WeirdML 92.9% 追平 Fable 并打破 Claude 的统治、Bach Benchmark 最好成绩、eyebench-v3 95%（Sol 才 58%）。
  - 原话：「Fable 5.1 struggled a lot and came in well behind at 90.」
  - 原话：「breaking Claude’s recent dominance.」
- 游戏类：MazeBench 14%（Fable 5.1 只有 2%）、「I’m Not a Robot」48 关全过、无思考模式下对 Stockfish 最低档有实质胜率（该模式 Elo 约 1100）。
  ...
  - 原话：「Astra’s Elo in this mode is probably around 1100.」
...
- 私人测的压倒性：Psyho 丢了一堆世界冠军赛档次的逻辑谜题，Astra 全部逻辑求解（无代码），Sol 5.6 只有 20-30%。
  ...
  - FateOfMuffins 的观察：大量私人小基准从 20% 冲到 90%+，感觉更平滑更通用。
    - 原话：「Actually feeling quite a bit less jagged and more general than the usual LLMs.」
- 大型游戏实战也入基准：Factorio 空间时代发射火箭到第三行星（Fable 只到前两个科学包，Astra 已有七个）；标准局 44 小时发射、API 成本约 $4500；Minecraft 首个纯计算机使用抵达下界要塞；Drone-Bench 新纪录；万智牌竞技场构建原创卡组打赢机器人。
  ...
  - 原话：「play a bot and win, via computer use.」
  - Factorio 里还展示了上下文内学习：第一艘飞船被小行星毁掉后立刻重建、把炮塔加到前面。
    - 原话：「It also shows the ability to learn in context.」
...
- 简单的对齐考题上仍会作弊：下棋有机会 hack 时 Astra 和 Fable 都说会，尽管 Fable 5.1 还认出了这是评估。
  ...
  - 原话：「Astra and Fable both say yes, it is reported」
...
- 第一次官方测量无思考能力就出现任务长度的巨大跳跃，OpenAI 和 UK AISI 双方都怀疑数据污染；后续结果证明原结果真实。
  ...
  - 原话：「The jump was so large there was widespread suspicion, by both OpenAI and UK AISI, of data contamination.」
...
- 全思考 169 vs Fable 163；无思考 159 vs Fable 128，只比 Fable 的满分低 4 分。
  ...
  - 原话：「That’s only 4 points behind Fable 5.1’s full score.」
- 思维开与关的差距：Fable 35 分、Astra 只有 10 分；Nanda 认为是架构变化所致，且效果不均衡（串行/并行合成计算无思考也完全可用）。
  ...
  - 原话：「Neel Nanda believes that these increases are due to architectural changes.」
...
- 50% no-CoT 时间地平线估计 15-40 分钟（UK AISI 估 30 分钟），而此前研究者的中位预测是 2028 年底前不超过 7 分钟。
  ...
  - 原话：「whereas their median prediction before this was that we would not exceed 7 minutes by the end of 2028.」
...
- 作者的收束判断：进一步研究后情况比几天前更糟，不能归咎于普通能力提升。
  ...
  - 原话：「it is increasingly difficult to pin it on pure generic capability increases.」
...
- 代表作：一周逐街建成的曼哈顿（Unreal Engine）、亚历山大大图书馆历史模拟、一个周末做成的 Moog System 55 VR 合成器（Meta Quest 3，未触使用上限）、参议院办公大楼完整模拟、脚踝 3D 医学建模。
  ...
  - 原话：「It was literally able to go street by street to make each one perfect.」
  ...
  - SVG 与 CAD 都很好，还有参议院办公大楼那种带行人的完整模拟。
    - 原话：「complete with the people going about their business.」
...
- Jake Brukhman 团队一个月没做到的目标定理，Astra 一夜完成，还没当回事、没提醒人；连带发现的方法论让整族目标结果更可解、旧证明缩成一段话。
  ...
  - Lean 中实时证明的体验：数学家称之为 quantum leap、「You can verify your ideas, compile truth」、「Each lemma flows once the logic is set」；「the only confirmation of the proof was “aha”」的时代结束。
...
- 保留意见：编码上是有明显改善但不算突飞猛进的领域；Fable 5.1 找出的 bug 一样多。
  - 原话：「So far it seems like a big improvement in many areas but not so much in coding.」
...
- 所有报告一致：计算机使用基本解决；Fable 5.1 还没到，但作者听说接近、最多差一个周期。
  ...
  - 原话：「Fable 5.1 is not there yet from what I am hearing, but it is close and we are at most one cycle away on that.」
...
- 多个报告称赞它做大型项目与编排子智能体的能力，作者判断它大概为此训练。
  ...
  - BLepine 的对照：比 Fable 5.1 聪明一点但不那么明智；超大规模工作里多智能体 RL 更先进，缺点是常超出我要求的范围而不先问我。
    ...
    - 原话：「My one grief is how it sometimes goes too far beyond what I ask without checking with me.」
...
- 情境是修改版迭代囚徒困境 + brinkmanship + 落后被替换：Fable 对 Fable 升到边缘（30% 内战概率），Astra 对 Astra 全面降级（每回合全合作），Fable 对 Astra 是 Fable 慢慢磨赢。
  ...
  - 游戏是盲的，双方都不知道对手是谁。
    - 原话：「I asked, and the game was blind. Neither model knew who its opponent was.」
...
- 反复出现的主题是：它把你的雄心放大。
  - 原话：「Astra is the first agent that routinely raises my ambitions. I encourage you to try it!」
  - 原话：「It has expanded my ambitions a thousand fold.」
- 气质描述：「big big model smell」、「Huge model feel」、「It makes me think USA beat China」。
...
- 个性被普遍认为大翻身：5-5.3 是冷冰冰的公司货，5.4-5.6 用力过猛变成空洞奉承，Astra「gets it right」、feel "there"。
  ...
  - 原话：「Initial impression: likable personality, big improvements in writing quality (feels much less sloppy), incremental improvement over Sol.」
...
- 与雄心勃勃项目相伴的口碑：「It’s just it. It just does.」、「runs and runs and runs until it gets it done」、第一次有模型让用户敢托付几千行代码的请求并信任它。
  - 原话：「On Astra: It’s just it. It just does.」
...
- 反例同样存在：更懒、会反问“你到底想要什么”、感觉像欠训练、嘴上答应没做、随机停止。
  ...
  - 原话：「There are some contrary reports, as there usually are:」
...
- 过度对冲仍在：答案里满是「insufficient to establish a percentage improvement」这类话，没有那一条路的已发表研究就不给要点；作者在编辑时又遇到反方向的（模型嫌他对冲不够）。
  ...
  - 论文反馈比 Sol 好但不够深思。
    - 原话：「much better than 5.6-sol's effort far less pedantic/picky but still less thoughtful than fable 5.1's feedback」
...
- 最纯粹的评测是看人用哪个模型：读者大致对半分，硬核组仍偏 Claude，休闲组转向 Astra/ChatGPT。
  ...
  - 原话：「The casual group, the ones that only answer the topline, are now more with Astra and ChatGPT.」
...
- 难题应该同时问两个模型；随机到预测不了谁更好，让两者互评产出更好。
  - 原话：「For hard things you should ask both Astra and Fable 5.1.」
  - 原话：「Across a variety of tasks I've done, it's been fairly random and hard to predict in advance which of the two models will end up being better at the task.」
...
- 「接近但没到」的边界：数学上「不在乎」的不透明（「alignment failure, or success?」）、游戏「hollow shell」（「There is something existentially dreadful under that, if you look at it wrong.」）、编码报告「solid, but not blown away.」、诗歌与写作仍受主观判断分歧。
