...
- 作者：OpenAI
- 类型：主题特刊
- 摘要：同一个模型、权重不变，只开启保留推理与上下文压缩，GPT-5.6 Sol 在 ARC-AGI-3 的成绩从 13.3% 升到 38.3%，输出 token 还少了约 6 倍。这是“harness 与上下文配置会改写能力表现”很少见的量化样本。
- 收藏于：—（主题特刊；清单更新于 2026-08-02）
- 抓取：飞书主题精选·图文版快照（evidence/概念源-260913，SHA256SUMS 冻结）
- 字数：10837
- 策展人按：原清单第 17–19 篇在算上下文的账，这篇给那笔账补上因果实验。建议放在第 19 篇之后：先看成本，再看两个开关怎么同时改写分数和账单。
