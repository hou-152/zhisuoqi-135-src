...
node scripts/check-learning-materials.mjs  # 363 项：Agent Loop 六章材料体检（不需要 serve、不调模型；含正文流「过渡句/关系句必须带逐字原文」＋来源链「58 篇 → 卡片 → 原始来源」）
...
node scripts/test-graph.mjs       # 215 项：全链路 Graph 固定响应端到端走查（六章＋单篇四判据＋夹具；补讲回原活动 · 章末独立门 · 暂停恢复 · 异常不记缺口）
...
| `knowledge/graph-260914/` | **全链路 Graph 索引**（构建产物）。**4533 节点 / 14281 边 / 105 条缺口**（2026-09-14 决策题接入：+456 个 Decision/DecisionReview 活动、−76 条「决策题待装配」缺口）；七层＝材料与出处 928 · 五类语义 538 · 公共知识与关系 1091 · 问题与课程编排 387 · 学习活动 1586 · 运行与记录 0（运行期才建）· 模型与规则 3。五种边：`provenance` / `knowledge` / `curriculum` / `transition` / `evidence`——**只有 `transition` 能驱动学习状态跳转**。契约见 `docs/总图视图契约-20260914.md`，回执见 `evidence/graph-260914/回执-全链路Graph-20260914.md`。`graph-report.md` 是自动生成的报告 | 直接读 JSON，或 serve 后开 `#graph=map` |
...
| `deploy/zhisuoqi-135/` | 公网版（单文件零服务端）。**09-13 减法后只剩 知识体系 + 内参**，对话三条路已随对话层删除；无服务端时交卷走机械兜底（只标 mech）。**已于 2026-09-14 push 上线**（`3d03a60` → `3a6de88` → `dfac7e8` → `05b6112` → `6f86913` → **`aca619d`**）：线上＝**1070 概念 / 681 依赖 ＋ 内参三期「日报集合」**（260914 / 260913 / 260912 共 26 篇；中间栏＝日期→分类→策展，主区＝日报卡），本地与线上逐字节一致（SHA-256 `df40b959…`，3,163,654 字节），对线上地址跑 `check-public.mjs` 全过 | <https://hou-152.github.io/zhisuoqi-135/> |
