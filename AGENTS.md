...
node scripts/check-learning-materials.mjs  # 241 项：Agent Loop 六章材料体检（不需要 serve、不调模型）
node scripts/test-learn-agent-loop.mjs     # 55 项：独立学习空间状态门（一题一判 · 费曼门 · 未过不解锁下一章 · 改复述清状态 · 返回恢复路线；费曼走固定响应）
node scripts/walk-learn-agent-loop.mjs agent  # 真模型走查一章（真调 /api/llm；不是断言脚本，是证据）
