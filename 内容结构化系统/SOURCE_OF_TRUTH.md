...
## 七、本实例的权威事实（AI 概念基本盘 · 2026-09-13）

| 找什么 | 权威位置 | 备注 |
| --- | --- | --- |
| 本批的真实状态、做法差异、已知问题 | `03-处理状态/处理状态总览.md` | **先读这一份**；静态数字不写死在这里 |
| 三个来源的登记 | `03-处理状态/来源注册表.csv` | `SRC-EXT-001`（Context 28 篇）· `SRC-EXT-002`（Harness 30 篇）· `SRC-EXT-003`（图鉴站产物） |
| 源快照（未改写） | `01-原始素材区/完整副本/` | 两份 `.md` 的 SHA-256 记在 `来源注册表.csv`；`图鉴站产物/` 是只读副本 |
| 单元生成脚本 | `07-脚本与工具/build-units.py` | 幂等：重跑会先清空 `02-内容单元库/*/*.md` |
| 主题地图与装配稿脚本 | `07-脚本与工具/build-maps-and-assemblies.py` | 7 张地图 · 2 份装配稿，引用的是真实文件名 |
| 壳用的两条策展线数据 | `05-主题地图/基本盘-工程范围轴.json` | 按图鉴站 ADR-0001 轴 01 切：Context Engineering 34 概念 / Harness Engineering 42 概念 |
| 上游产物（只读，不归本工程管） | `/Users/housibo/Documents/ChatGPT/context+harness 网站/` | 「Context × Harness 图鉴」Astro 站；它有**自己的** `SOURCE_OF_TRUTH.md`，两边冲突时各管各的 |

**版本规则**：本工程只管「五类语义单元」；概念的定义与关系以图鉴站概念卡为上游真源，
本工程只搬运不改写。上游概念卡更新后，重跑 `build-units.py` 即可同步（脚本不做增量合并）。
